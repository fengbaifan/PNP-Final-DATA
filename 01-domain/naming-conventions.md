...
| `03-processing/` | 摄入至关系及后续发现的过程记录，摄入处理结果、来源定位及候选 | 任务包 process/stages.md 记录摄入处理，process/knowledge.md 分节记录知识阶段；具体交接见 pipeline |
| `04-knowledge/` | 知识元、关系、证据、结构及对应当前结果 | 第三至六阶段成果；后续发现启动后保存知识成果，过程统一在 03-processing |
...
先核对目录实际内容、调用者和来源/证据边界；从 01 的契约修订开始，再依次处理业务目录及系统依赖。迁移映射与验证记入既有 CHANGELOG.md，不另建映射报告。Windows 移动或删除前验证绝对路径在仓库内，联接不递归进入目标。修复活跃引用，历史记录保留当时路径。Git 提交、推送按明确授权执行。
