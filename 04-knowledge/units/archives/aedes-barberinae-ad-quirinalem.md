---
title: 奎里纳尔山的巴贝里尼宫（Aedes Barberinae ad Quirinalem）
name_en: Aedes Barberinae ad Quirinalem
type: archive
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图版目录；印刷页xii；OCR L49–50
  sentence_summary: 图版09目录将其记为载体文献。
  original_quotes:
  - source_span: lines 49–50
    text: '9 Guido Abbatini: Frontispiece of Girolamo Teti: Aedes Barberinae ad

      Quirinalem, Rome 1642, with view of Palazzo Barberini 56'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 49–50; 章前：图版目录；印刷页xii
- citation: The Metropolitan Museum of Art, Aedes Barberinae, object 48.106.2. https://www.metmuseum.org/art/collection/search/344493.
    Accessed 2026-09-15.
  location: 完整Artwork Details：责任者、出版地、年份、媒介与具体印本字段；未读全书
  sentence_summary: Girolamo Teti著，Mascardi于罗马出版，1642；48.106.2及尺寸属于大都会所藏印本。
- citation: Universitätsbibliothek LMU München, Aedes Barberinae ad Quirinalem a comite Hieronymo Tetio descriptae,
    item 12211. https://epub.ub.uni-muenchen.de/12211/. Accessed 2026-09-15.
  location: 完整目录记录、题名、出版项、语言、索书号、URN及印本流传说明；未读400MB全文
  sentence_summary: 作者拉丁名Hieronymus Tetius，罗马Mascardus出版，1642，拉丁语；Cim.36 (=2 Art.277)为该馆印本索书号。
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#机构身份与书目版本责任集中补足
relations:
- relation_type: authored_by
  target: persons/girolamo-teti.md
  evidence_ref:
    doc_id: archives-aedes-barberinae-ad-quirinalem
    source_file: https://www.metmuseum.org/art/collection/search/344493
    source_span: Artwork Details：Author、Date
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 馆方明确的文字作者，不是全书插图刻版者
  role: 文字作者
  time: 1642出版
- relation_type: published_by
  target: institutions/mascardi-rome.md
  evidence_ref:
    doc_id: archives-aedes-barberinae-ad-quirinalem
    source_file: https://www.metmuseum.org/art/collection/search/344493
    source_span: Artwork Details：Publisher、Published in、Date
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 1642年罗马版出版项；不把机构名称扩写成特定自然人
  role: 出版机构
  time: '1642'
- relation_type: located_at
  target: places/rome.md
  evidence_ref:
    doc_id: archives-aedes-barberinae-ad-quirinalem
    source_file: https://www.metmuseum.org/art/collection/search/344493
    source_span: Artwork Details：Published in、Date
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 出版地，非印本保管地
  role: 出版地点
  time: '1642'
---

## 内容

### 描述

**中文：** 吉罗拉莫·泰蒂的《奎里纳尔山的巴贝里尼宫》于1642年在罗马出版；本书图版9收录其卷首图。

**English:** Girolamo Teti’s Aedes Barberinae ad Quirinalem was published in Rome in 1642; plate 9 reproduces its frontispiece.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Aedes Barberinae ad Quirinalem | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版09角色 | 载体文献 | S1 |
| 图版09相关对象 | [《巴贝里尼宫》卷首图](../works/guido-abbatini-frontispiece-of-aedes-barberinae-ad-quirinalem.md)；载体文献 | S1 |
| 责任者 | [吉罗拉莫·泰蒂](../persons/girolamo-teti.md) | S1 |
| 出版地点 | [罗马](../places/rome.md) | S1 |
| 出版年 | 1642 | S1 |

### 版本与书目身份

| 字段 | 值 | 证据 |
|---|---|---|
| 完整拉丁题名 | Aedes Barberinae ad Quirinalem a comite Hieronymo Tetio descriptae | S3 |
| 作者拉丁名 | Hieronymus Tetius | S3 |
| 语言 | 拉丁语 | S3 |
| 媒介 | 印刷书，含版刻插图 | S2 |
| 出版机构 | [罗马马斯卡尔迪](../institutions/mascardi-rome.md) | S2；S3 |
| Met印本记录号 | 48.106.2 | S2 |
| LMU印本索书号 | Cim.36 (=2 Art.277) | S3 |
| LMU数字印本URN | urn:nbn:de:bvb:19-epub-12211-6 | S3 |


## 关系与证据

### 身份与外部链接

[Met 1642年版印本记录](https://www.metmuseum.org/art/collection/search/344493)（S2）
[LMU 1642年版印本记录](https://epub.ub.uni-muenchen.de/12211/)（S3）


### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 作者（`authored_by`） | [吉罗拉莫·泰蒂（Girolamo Teti）](../persons/girolamo-teti.md) | 时间：1642出版；角色：文字作者；范围：馆方明确的文字作者，不是全书插图刻版者；证据：[来源](https://www.metmuseum.org/art/collection/search/344493)；archives-aedes-barberinae-ad-quirinalem；Artwork Details：Author、Date |
| → published_by（`published_by`） | [罗马马斯卡尔迪出版机构（Mascardi (Rome)）](../institutions/mascardi-rome.md) | 时间：1642；角色：出版机构；范围：1642年罗马版出版项；不把机构名称扩写成特定自然人；证据：[来源](https://www.metmuseum.org/art/collection/search/344493)；archives-aedes-barberinae-ad-quirinalem；Artwork Details：Publisher、Published in、Date |
| → 位于（`located_at`） | [罗马（Rome）](../places/rome.md) | 时间：1642；角色：出版地点；范围：出版地，非印本保管地；证据：[来源](https://www.metmuseum.org/art/collection/search/344493)；archives-aedes-barberinae-ad-quirinalem；Artwork Details：Published in、Date |
| ← 包含成员（`contains`，反向投影） | [《巴贝里尼宫》卷首图（Frontispiece of Aedes Barberinae ad Quirinalem）](../works/guido-abbatini-frontispiece-of-aedes-barberinae-ad-quirinalem.md) | 时间：1642；角色：卷首图；范围：原书图版明确的卷首图与1642载体；未核剑桥具体印本号；原断言与证据见发出端卡片“《巴贝里尼宫》卷首图（Frontispiece of Aedes Barberinae ad Quirinalem）”：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 49–50; 章前：图版目录；印刷页xii |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
