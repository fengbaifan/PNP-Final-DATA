---
title: "《当今》（1627）（L'Hoggidi (1627)）"
name_en: "L'Hoggidi (1627)"
type: archive
created: 2026-09-09
updated: 2026-09-12
evidence_status: source_backed
relations:
  - {"relation_type":"authored_by","target":"persons/abate-lancellotti.md","note":"第一章脚注署作者；初刊年份异文见正文，不作为作者关系的依据。","evidence_ref":{"doc_id":"patrons-and-painters","source_file":"02-sources/02-Markdown/01_CHP-1.md","source_span":"lines 45–47; print pp. 3"},"review_status":"evidence_backed_relation","relation_source":"explicit","bidirectional_required":false}
sources:
- citation: 'Francis Haskell, Patrons and Painters, revised and enlarged ed. (New Haven and London: Yale University Press, 1980; this printing 2006), ch. 1, pp. 3.'
  location: 第一章；印刷页 3；OCR L45–47
  sentence_summary: Abate Lancellotti 的著作。脚注记其 1627 年在 Venice 首刊，之后多次重印，并赞颂 Barberini 时期；不把重印全部定在 1627 年。只据引用登记书目，未读全书。
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/01_CHP-1.md
    source_span: lines 45–47; print pp. 3
- citation: "LANCELLOTTI, Secondo, Dizionario Biografico degli Italiani. https://www.treccani.it/enciclopedia/secondo-lancellotti_(Dizionario-Biografico)/. Accessed 2026-09-10."
  location: "正文 Hoggidì 出版与献辞段"
  sentence_summary: "1623 年出版及 1627 年重印与本章 1627 首刊说不同，保留版本冲突。"
- citation: "Emilio Russo, LANCELLOTTI, Secondo, Dizionario Biografico degli Italiani, vol.63 (2004). https://www.treccani.it/enciclopedia/secondo-lancellotti_(Dizionario-Biografico)/. Accessed 2026-09-10."
  location: "Hoggidì第一部刊印、献辞、重印及1636第二部段；REV-052定向复读"
  sentence_summary: "第一部1623年Guerigli威尼斯刊，1627/1630/1637重印；1623年10月24日献辞；第二部1636年另刊，具体1627刊本未读。"
---

## 内容

### 描述

**中文：** 《当今》是塞孔多·兰切洛蒂论述古今优劣的著作。本卡固定记录本章提到的1627年刊本线索：Haskell称其为首刊，外部传记则将1627年列为第一部的重印。作者已经明确，刊本实物仍未核定。

**English:** L’Hoggidì is Secondo Lancellotti’s work on the relative merits of ancient and modern times. This card retains the chapter’s reference to a 1627 edition: Haskell calls it the first publication, whereas the external biography identifies 1627 as a reprint of part one. The author is identified; the physical edition remains unexamined.

### 题名与责任者

| 字段 | 内容 | 依据 |
|---|---|---|
| 本章题名 | L’Hoggidi | S1 |
| 第一部完整原题 | L’hoggidì overo Il mondo non peggiore ne più calamitoso del passato | S3；拼写随来源 |
| 中文题名 | 《当今》；项目暂译 | 原题翻译 |
| 英文显示名 | L’Hoggidì (1627 edition referenced in chapter 1) | 对象范围说明，不称英语译本 |
| 作者 | [塞孔多·兰切洛蒂（Secondo Lancellotti）](../persons/abate-lancellotti.md) | S1署Abate Lancellotti；S3明确全名 |
| 文献语言 | 意大利语题名及论述；具体刊本待核 | S3 |

### 版本与内容

| 字段 | 内容 | 依据 |
|---|---|---|
| 第一部首刊年 | 1623 | S3 |
| 刊印地点 | [威尼斯（Venice）](../places/venice.md) | S1、S3 |
| 首刊印刷者 | Guerigli | S3；不据此自动断定所有重印相同 |
| 本章所引年份 | 1627 | S1；S3记为重印年 |
| 其他重印年份 | 1630、1637 | S3 |
| 献辞对象 | [乌尔班八世（Urban VIII）](../persons/urbano-viii.md) | S3 |
| 献辞日期 | 1623年10月24日，威尼斯 | S3；献辞不是印行日 |
| 第一部内容 | 50则disinganni，以古今比较反对贬低当代 | S3，传记概述；未通读原著 |
| 第二部 | Gl’ingegni non inferiori a’ passati，1636 | S3；不与1627所指刊本合并 |
| 原件／馆藏号 | 尚未确认本章使用的具体1627册 | 当前限制 |

### 本章相关内容

第3页脚注L45–47（S1）说该书赞颂巴贝里尼时期，1627年在威尼斯首刊并屡次重印。保留原章表述，当前版本字段采用S3区分1623首刊和1627重印；这解决了两种年份所指事件的区分，尚未解决本章实际引用册的版次与页码。

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 作者（`authored_by`） | [塞孔多·兰切洛蒂（Secondo Lancellotti）](../persons/abate-lancellotti.md) | 第一章脚注署作者；初刊年份异文见正文，不作为作者关系的依据；证据：[来源](../../../02-sources/02-Markdown/01_CHP-1.md)；patrons-and-painters；lines 45–47; print pp. 3 |

### 身份与外部链接

本著作／具体刊本尚无确认QID或配对Wikipedia页。[作者传记](https://www.treccani.it/enciclopedia/secondo-lancellotti_(Dizionario-Biografico)/)支持版本信息；作者的QID不充当本书QID。REV-034版本待证状态保留，未借字段补足自动提升身份状态。

### 证据与未决项

S1是转引入口；S2是先前版本核查；S3是本轮有范围的复核，三者不代表读过原件。未核1627题名页、完整出版说明、页码或馆藏标识，不强填ISBN或QID。当前完成版本字段补足及冲突区分，尚非刊本鉴定完成。

[过程证据](../../../03-processing/patrons-and-painters-chp-1/process/enrichment-evidence.jsonl)按稳定键及REV-052定位。
