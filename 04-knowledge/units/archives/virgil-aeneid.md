---
title: 维吉尔《埃涅阿斯纪》（Virgil’s Aeneid）
name_en: Virgil’s Aeneid
type: archive
evidence_status: source_backed
sources:
- citation: Macerata Musei, Enea racconta a Didone la caduta di Troia, inventory 131a. https://musei.macerata.it/opera/enea-racconta-a-didone-la-caduta-di-troia/.
    Accessed 2026-09-15.
  location: 完整作品说明及技术字段；题材首段、画廊委托／交付段、作者、日期、媒介、尺寸、位置和库存号
  sentence_summary: 班比尼的131a为1712年布面油画，300×300 cm，约1712–1713交付画廊；画面依据维吉尔《埃涅阿斯纪》第一卷的宴会场景，馆方列陈列位置为布奥纳科尔西宫埃涅阿斯画廊14室。
created: '2026-09-15'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#班比尼馆藏作品与文学来源
relations:
- relation_type: authored_by
  target: persons/virgil.md
  evidence_ref:
    doc_id: macerata-131a
    source_file: https://musei.macerata.it/opera/enea-racconta-a-didone-la-caduta-di-troia/
    source_span: 题材首段：Eneide di Virgilio
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 文学作者
  scope: 《埃涅阿斯纪》作品层级
---

## 内容

### 描述

**中文：** 《埃涅阿斯纪》是维吉尔的文学作品；班比尼馆藏画131a采用其中第一卷的宴会场景。本条登记作品身份，未指定某一出版版本。

**English:** The Aeneid is a literary work by Virgil. Bambini’s painting 131a draws on the banquet scene in Book I. This record identifies the work without specifying a published edition.

### 题名与文献身份

| 字段 | 值 | 证据 |
|---|---|---|
| 中文题名 | 埃涅阿斯纪 | S1；项目译名 |
| 英文题名 | Aeneid | S1；题名对应 |
| 来源所用题名 | Eneide | S1，题材段 |
| 作者 | [维吉尔](../persons/virgil.md) | S1，Eneide di Virgilio |
| 本次涉及篇章 | 第一卷／Book I | S1 |
| 版本范围 | 未指定出版版本 | S1，作品层级 |

### 艺术取材

| 作品 | 所用内容 | 证据 |
|---|---|---|
| [埃涅阿斯向狄多讲述特洛伊的陷落](../works/bambini-aeneas-tells-dido-fall-of-troy.md) | 宴会结束后狄多邀请埃涅阿斯讲述往事 | S1，题材段 |

## 关系与证据

### 身份与外部链接

[Macerata Musei：131a](https://musei.macerata.it/opera/enea-racconta-a-didone-la-caduta-di-troia/)（S1）。

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 来源（`source_of`，反向投影） | [埃涅阿斯向狄多讲述特洛伊的陷落（Aeneas Tells Dido of the Fall of Troy）](../works/bambini-aeneas-tells-dido-fall-of-troy.md) | 角色：文学题材来源；范围：第一卷宴会场景；原断言与证据见发出端卡片“埃涅阿斯向狄多讲述特洛伊的陷落（Aeneas Tells Dido of the Fall of Troy）”：[来源](https://musei.macerata.it/opera/enea-racconta-a-didone-la-caduta-di-troia/)；macerata-131a；题材首段：primo libro dell’Eneide di Virgilio |
| → 作者（`authored_by`） | [维吉尔（Virgil）](../persons/virgil.md) | 角色：文学作者；范围：《埃涅阿斯纪》作品层级；证据：[来源](https://musei.macerata.it/opera/enea-racconta-a-didone-la-caduta-di-troia/)；macerata-131a；题材首段：Eneide di Virgilio |
