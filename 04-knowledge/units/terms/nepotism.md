---
title: "教皇亲族任用（Nepotism）"
name_en: "Nepotism"
type: term
created: 2026-09-09
updated: 2026-09-14
evidence_status: source_backed
sources:
- citation: 'Francis Haskell, Patrons and Painters, revised and enlarged ed. (New Haven and London: Yale University Press, 1980; this printing 2006), ch. 1, pp. 4.'
  location: 第一章；印刷页 4；OCR L56–70
  sentence_summary: 本章指新教皇将亲友客户带入有利职位的安排；私人领地式任用已改变，收入和赞助地位仍紧密依赖统治更替。
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/01_CHP-1.md
    source_span: lines 56–70; print pp. 4
  original_quotes:
  - source_span: "lines 56–70"
    text: |2-
      These ‘speedy revolutions’ and the consequent rise of new families moulded the
      patterns of art patronage. As successive popes came to the throne they surrounded
      themselves with a crowd of relatives, friends and clients who poured into Rome from
      all over Italy to seize the many lucrative posts that changed with each change of govern­
      ment. These men at once began to build palaces, chapels and picture galleries. As patrons
      they were highly competitive, anxious to give expression to their riches and power as
      quickly as they could and also to discomfort their rivals. After the Pope died they were
      often disgraced, and in any case their vast incomes came to a sudden end, for nepotism
      no longer took the form of private empires carved out of the Church’s territory. ‘There
      is no situation more difficult or more dangerous’, said Pope Gregory XV, who certainly
      knew what he was talking about,2 ‘than that of a Pope’s nephew after the death of his
      uncle.’ With the end of their incomes went the end of their positions as leading patrons.
      It was especially noted of Cardinal Alessandro Peretti-Montalto, nephew of Pope Sixtus
      V, that he was still respected and loved even after the death of that Pope, and that
      artists continued to work for him.3 This was evidently not the usual state of affairs.
- citation: "Wikipedia (en), Nepotism, revision 1369375299. https://en.wikipedia.org/wiki/Nepotism. Accessed 2026-09-10."
  location: "导言身份段；REV-034 初步对齐，非全文补足"
  sentence_summary: "亲族优待概念，英文页说明教皇侄辈任用的词源；本章限定为教皇家族语境。"
- citation: "Wikidata, Q161165, revision 2534275107. https://www.wikidata.org/wiki/Q161165. Accessed 2026-09-10."
  location: "labels／descriptions／P31／适用身份字段及 enwiki sitelink；判断范围见正文"
  sentence_summary: "亲族优待概念，英文页说明教皇侄辈任用的词源；本章限定为教皇家族语境。"
- citation: "Wikipedia (en), Nepotism, revision 1369375299. https://en.wikipedia.org/wiki/Nepotism. Accessed 2026-09-10."
  location: "全文：定义、词源、类型、案例、注释和书目；REV-052"
  sentence_summary: "采用定义与历史词源线索；现代政治、企业和娱乐案例不移植到本章或新建对象。"
- citation: "Wikidata, Q161165, revision 2534275107. https://www.wikidata.org/wiki/Q161165. Accessed 2026-09-10."
  location: "英／意 labels，P279、P138 的 rank、引用与目标标签核对"
  sentence_summary: "保留规范名称；无引用的 P279 与 P138 不能直接作为概念层级和词源证据，尤其 P138 指向皇帝 Julius Nepos，未采用。"
- citation: "Treccani, Nepotismo, Vocabolario on line. https://www.treccani.it/vocabolario/nepotismo/. Accessed 2026-09-10."
  location: "词条全文，词源与两个义项"
  sentence_summary: "区分教皇史中的亲族授职与扩大到亲友优待的广义用法。"
- citation: "Judy Nadler and Miriam Schulman, Favoritism, Cronyism, and Nepotism, Markkula Center for Applied Ethics, Santa Clara University. https://www.scu.edu/government-ethics/resources/a-primer-on-government-ethics/favoritism-cronyism-and-nepotism/. Accessed 2026-09-10."
  location: "正文全文；采用定义比较部分，不采用其法制现况为本章依据"
  sentence_summary: "现代英文用法将家庭优待与朋友／同伴优待分开；这里的政治 patronage 也不能直接代替本书的艺术赞助。"
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-terms-nepotism
---

## 内容

### 描述

**中文：** 教皇亲族任用（Nepotism）在本章指与教皇更替相关的亲族授职、收入及赞助机会。作者同时讨论友人和依附者组成的周边网络，但亲缘、职位获得与实际艺术委托是不同事实，需各自有证据。[S1、S4、S6–7]

**English:** In this chapter, nepotism concerns relatives' appointments, incomes and opportunities for patronage under successive popes. Haskell also discusses a surrounding network of friends and clients, but kinship, appointment and an actual artistic commission are distinct facts requiring separate evidence. [S1, S4, S6–7]

### 名称与定义

| 字段 | 内容 | 依据 |
|---|---|---|
| 本章中文名 | 教皇亲族任用 | 限定历史语境的项目用名 |
| 英文名 | Nepotism | S4、S5 |
| 意大利语名 | nepotismo；罕见变体 nipotismo | S6；S5 意大利语标签 |
| 通用中文对应 | 任人唯亲、裙带关系；不机械替换本章所有 patronage | 项目译释，语境见 S6–7 |
| 词源 | 意大利语 nepote／nepotismo，与拉丁语 nepos 的亲族词义相关 | S4、S6 |
| 教皇史义项 | 某些教皇向亲属，尤其侄辈，授予职位与收益的做法 | S6 |
| 广义用法 | 亲属优待；意大利语扩展义也包括朋友，现代英文定义常与 cronyism 分开 | S6、S7 |
| 适用年代 | 本章为十七世纪罗马语境；词典的历史义覆盖更长时期，不作为制度精确起止年 | S1、S6 |

### 本章中的运行条件

| 项目 | 内容 | 依据 |
|---|---|---|
| 职位与收入 | 教皇更替带来职位和受益网络变化，亲属、朋友与依附者进入罗马 | S1，L56–60 |
| 艺术赞助 | 新受益者建造宫殿、小堂和画廊，以财富与地位展开竞争 | S1，L60–62 |
| 统治更替风险 | 教皇死后收入与赞助地位可能终止，不把亲族身份视为永久保障 | S1，L62–67 |
| 形态变化 | 作者称此时已不再主要以分割教会领土建立私人领地的形式运行 | S1，L63–64；是作者对本章时期的说明 |
| 反例／差异 | [蒙塔尔托枢机](../persons/alessandro-peretti-montalto.md)在西斯笃五世死后仍获尊重并继续用艺术家，作者将其视为非常态 | S1，L68–70 |
| 制度后续线索 | 1692 年 Innocent XII 的 Romanum decet Pontificem 限制亲族利益；具体条款尚未核原文 | S4，不能写成从此彻底消失 |

### 相邻概念与研究

| 项目 | 区别或入口 | 依据 |
|---|---|---|
| Favoritism | 广义偏爱／优待；不能自动证明存在血亲关系 | S7 |
| Cronyism | 朋友、同伴或内部网络优待；与狭义 nepotism 区分 | S7 |
| 艺术赞助 | 提供委托、收入或保护的关系；不必依赖亲缘，也不天然等于裙带行为 | 本章语境与 S7 的语义比较 |
| 亲缘事实 | 是人物之间的身份关系；只有额外授职、收益和情境证据才支持具体 nepotism 判断 | 采用边界 |
| 研究线索 | Gianvittorio Signorotto、Maria Antonietta Visceglia 编，Court and Politics in Papal Rome, 1492–1700（2002） | S4 所列书目；出版社前置页访问超时，未读全书 |
| 定义比较 | Nadler、Schulman 的大学伦理中心文章 | S7 实际正文已读；现代规范评价不回填为历史定性 |

## 关系与证据

### 关系记录
暂无正式关系。

### 身份与外部链接

- [Wikidata Q161165](https://www.wikidata.org/wiki/Q161165)
- [Wikipedia（en）Nepotism](https://en.wikipedia.org/wiki/Nepotism)
