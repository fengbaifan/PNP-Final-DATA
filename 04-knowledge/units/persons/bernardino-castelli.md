---
title: 贝尔纳迪诺·卡斯泰利（Bernardino Castelli）
name_en: Bernardino Castelli
type: person
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图版目录；印刷页xv；OCR L137–137
  sentence_summary: 图版59b将其列为《泰奥多罗·科雷尔》的创作者。
  original_quotes:
  - source_span: lines 137–137
    text: '59 a Canova: Amadeo Swajer (Museo Correr, Venice) b Bernardino Castelli: Teodoro Correr (Museo Correr,
      Venice)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 137–137; 章前：图版目录；印刷页xv
- citation: Nicola Ivanoff, CASTELLI, Bernardino, Dizionario Biografico degli Italiani 21 (1978). https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/.
    Accessed 2026-09-15.
  location: 完整传记正文及附录书目；起首、迁居段、卒年段、Correr藏品段
  sentence_summary: 1750年出生，1810年卒于威尼斯；1775年迁帕多瓦，Correr肖像与1795年复制版画分别记载。
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#画家与收藏家姓名师承及肖像关系
relations:
- relation_type: located_at
  target: places/padua.md
  evidence_ref:
    doc_id: persons-bernardino-castelli
    source_file: https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/
    source_span: 1775年转往Padova段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 传记明确地点事件
  role: 迁居
  time: '1775'
- relation_type: located_at
  target: places/venice.md
  evidence_ref:
    doc_id: persons-bernardino-castelli
    source_file: https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/
    source_span: Morì a Venezia段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 传记明确地点事件
  role: 去世
  time: '1810-02-24'
---

## 内容

### 描述

**中文：** 贝尔纳迪诺·卡斯泰利（1750–1810）是肖像与宗教画家；本书图版59b收录其泰奥多罗·科雷尔肖像。

**English:** Bernardino Castelli (1750–1810) was a portrait and religious painter; plate 59b reproduces his portrait of Teodoro Correr.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 惯用名 | Bernardino Castelli | S1 |
| 全名 | Bernardino Castelli | S2 |
| 别称 | pittore delle belle Madonnine／“美丽圣母像的画家” | S2 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 作品与活动记录

| 字段 | 值 | 证据 |
|---|---|---|
| 图版59b角色 | 创作 | S1 |
| 图版59b相关对象 | [泰奥多罗·科雷尔肖像](../works/bernardino-castelli-teodoro-correr.md)；创作者 | S1 |

### 基本信息

| 字段 | 值 | 证据 |
|---|---|---|
| 出生日期 | 1750-06-15 | S2 |
| 死亡日期 | 1810-02-24 | S2 |
| 去世地 | [威尼斯](../places/venice.md) | S2 |


### 身份标签

| 字段 | 值 | 证据 |
|---|---|---|
| 身份 | 肖像画家 | S2 |
| 身份 | 宗教画家 | S2 |


### 职业履历

| 字段 | 值 | 证据 |
|---|---|---|
| 1775 | 迁居[帕多瓦](../places/padua.md)，继续肖像绘画 | S2 |


## 关系与证据

### 身份与外部链接

[实际采用的身份资料](https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/)（S2）


### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 位于（`located_at`） | [帕多瓦（Padua）](../places/padua.md) | 时间：1775；角色：迁居；范围：传记明确地点事件；证据：[来源](https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/)；persons-bernardino-castelli；1775年转往Padova段 |
| → 位于（`located_at`） | [威尼斯（Venice）](../places/venice.md) | 时间：1810-02-24；角色：去世；范围：传记明确地点事件；证据：[来源](https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/)；persons-bernardino-castelli；Morì a Venezia段 |
| ← 所创作对象（`creator_of`，反向投影） | [泰奥多罗·科雷尔肖像（Portrait of Teodoro Correr）](../works/bernardino-castelli-teodoro-correr.md) | 角色：肖像作者；范围：原书该图版明确列出的作者；原断言与证据见发出端卡片“泰奥多罗·科雷尔肖像（Portrait of Teodoro Correr）”：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 137–137; 章前：图版目录；印刷页xv |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
