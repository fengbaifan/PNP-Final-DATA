---
title: "奥诺拉托·吉尼（Onorato Gini）"
name_en: "Onorato Gini"
type: person
created: 2026-09-09
updated: 2026-09-14
evidence_status: source_backed
sources:
- citation: 'Francis Haskell, Patrons and Painters, revised and enlarged ed. (New Haven and London: Yale University Press, 1980; this printing 2006), ch. 1, pp. 11.'
  location: 第一章；印刷页 11；OCR L415–418
  sentence_summary: 萨伏依驻罗马代表，1666 年信件转述科尔托纳拒绝自行提出画题；信由 Claretta 摘述，非本次直接读取原件。
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/01_CHP-1.md
    source_span: lines 415–418; print pp. 11
  original_quotes:
  - source_span: "lines 415–418"
    text: |2-
      1 Letter from the Savoy Resident in Rome, Onorato Gini, in 1666, summarised bv Claretta, 1885,
      p. 516: ‘ma prima patco apposto dal Cortona era ch’egli non voleva indursi a far ver una proposta [as
      regards subject], allegando chc non avevane fatta alcuna in tutta la vita e che “questo sarebbe un non mai
      volere il quadro” ’.
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-persons-onorato-gini
---

## 内容

### 描述

**中文：** 萨伏依驻罗马代表，1666 年信件转述科尔托纳拒绝自行提出画题；信由 Claretta 摘述，非本次直接读取原件。

**English:** Savoy's Roman representative reported Cortona's refusal to choose a subject in a 1666 letter. Claretta's summary is the cited intermediary; the original was not directly read.

### 已有信息

| 项目 | 内容 | 依据 |
|---|---|---|
| 名称／对应英文 | Onorato Gini | S1 |
| 职能与文献 | 萨伏依驻罗马代表；1666 年信经 Claretta 转述 | S1 |

### 补充信息

| 字段 | 当前结论 | 依据／边界 |
|---|---|---|
| 全名／登记名 | Onorato Gini | 章内正文、脚注或已读刊本文本；未把检索名形自动当别名 |
| 名称变体／称号 | Onorato Gini | 保留原文拼写、OCR纠正或称号边界 |
| 身份标签 | 都灵驻罗马代理人；通信者；委托协调者 | 标签可并列；不据单一行为扩张职业身份 |
| 已知活动时段与地点 | 1666年；罗马—萨伏依宫廷 | 仅在现有证据范围内 |
| 履历／本章活动 | 转达Pietro da Cortona拒绝依指定题材作画的理由；现存证据为Claretta刊布链和同年纪年材料。 | 章内来源及本卡新增来源；具体支持范围见sources |
| 生卒、国籍、亲缘、教育、完整职业履历 | 当前未获可归属于本人的可靠资料 | 已检索未解决；不表示该人物没有这些属性 |

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 所著文献（`author_of`，反向投影） | [奥诺拉托·吉尼关于科尔托纳选题的信（1666）（Onorato Gini’s Letter on Cortona’s Choice of Subjects (1666)）](../archives/gini-cortona-letter-1666.md) | 本章署 Gini，信经 Claretta 概述；原断言与证据见发出端卡片“奥诺拉托·吉尼关于科尔托纳选题的信（1666）（Onorato Gini’s Letter on Cortona’s Choice of Subjects (1666)）”：[来源](../../../02-sources/02-Markdown/01_CHP-1.md)；patrons-and-painters；lines 415–418; print pp. 11 |
