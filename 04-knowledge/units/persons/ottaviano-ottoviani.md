---
title: 奥塔维亚诺·奥托维亚尼（Ottaviano Ottoviani）
name_en: Ottaviano Ottoviani
type: person
evidence_status: source_backed
sources:
- citation: Bibliothèque nationale de France, La galerie Mazarine. https://www.bnf.fr/en/mazarin-gallery. Accessed 2026-09-14.
  location: 完整正文、历史沿革及图注；尤其La galerie d’apparat du palais Mazarin
  sentence_summary: 1644年Mazarin委托François Mansart扩建上下画廊；上层穹顶1646–1647年由Giovanni Francesco Romanelli及作坊绘制，列Paolo Gismondi，镀金灰泥责任者Ottaviano Ottoviani。页面中的未来修复计划不作当前状态。
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#马扎然画廊与版画责任
---

## 内容

### 描述

**中文：** 法国国家图书馆画廊史所列的马扎然画廊穹顶镀金灰泥装饰责任者。

**English:** The artist credited in the BnF gallery history with the gilded stucco decoration of the Galerie Mazarine vault.

### 姓名

| 字段 | 值 | 证据 |
|---|---|---|
| 来源姓名 | Ottaviano Ottoviani／奥塔维亚诺·奥托维亚尼 | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 作品与角色

| 字段 | 值 | 证据 |
|---|---|---|
| 镀金灰泥装饰 | [马扎然画廊穹顶装饰](../works/mazarin-gallery-vault-decoration.md) | S1 |

## 关系与证据

### 身份与外部链接

[BnF穹顶灰泥责任记录](https://www.bnf.fr/en/mazarin-gallery)（S1，按来源拼写）。

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 所创作对象（`creator_of`，反向投影） | [马扎然画廊穹顶装饰（Vault Decoration of the Galerie Mazarine）](../works/mazarin-gallery-vault-decoration.md) | 角色：镀金灰泥责任者；范围：上层画廊装饰整体的分工；1646–1647仅明确用于壁画，未细化灰泥完工日期；原断言与证据见发出端卡片“马扎然画廊穹顶装饰（Vault Decoration of the Galerie Mazarine）”：[来源](https://www.bnf.fr/en/mazarin-gallery)；src-74f81a2927eecba5；La galerie d’apparat du palais Mazarin：委托、绘制者、灰泥与上层穹顶 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
