---
title: 埃伯哈德·茨维克（Eberhard Zwicker）
name_en: Eberhard Zwicker
type: person
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图片来源；印刷页xvi；OCR L172–172
  sentence_summary: 本书图片来源列明的供片者。
  original_quotes:
  - source_span: lines 172–172
    text: '18a; University Library, Cambridge: 9, 57a; Villani, Bologna: 28a; Eberhard Zwicker, Wiirzburg: 50.'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 172–172; 章前：图片来源；印刷页xvi
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md
---

## 内容

### 描述

**中文：** 本书图片来源列明的供片者。

**English:** A photographic supplier named in the book.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 惯用名 | Eberhard Zwicker | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 作品与活动记录

| 字段 | 值 | 证据 |
|---|---|---|
| 所供图版 | 50 | S1 |
| 供片对应对象 | [维尔茨堡宫楼梯顶画局部](../works/tiepolo-detail-from-fresco-on-ceiling-of-staircase-in-residenz.md)；图版50复制图像 | S1 |

## 关系与证据

### 关系记录

暂无正式关系。内容表中的知识元链接表示有来源的关联记录。

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
