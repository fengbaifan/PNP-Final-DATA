---
title: "丹尼斯·马洪（Denis Mahon）"
name_en: "Denis Mahon"
type: person
created: 2026-09-09
updated: 2026-09-14
evidence_status: source_backed
sources:
- citation: 'Francis Haskell, Patrons and Painters, revised and enlarged ed. (New Haven and London: Yale University Press, 1980; this printing 2006), ch. 1, pp. 12.'
  location: 第一章；印刷页 12；OCR L467–468
  sentence_summary: Haskell 所记时期 Sacchi 祭坛稿在 Mahon 藏；有 Colnaghi 图录线索。
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/01_CHP-1.md
    source_span: lines 467–468; print pp. 12
  original_quotes:
  - source_span: "lines 467–468"
    text: |2-
      are also recorded. One of these, for an altarpiece in the Capuchin church in Rome, recently passed through
      a London gallery (Colnaghi’s, May-June 1961, No. 2) and is now in the collection of Mr Denis Mahon.
- citation: 'Francis Haskell, Patrons and Painters, revised and enlarged ed. (New Haven and London: Yale University Press, 1980; this printing 2006), ch. 1, pp. 17.'
  location: 第一章；印刷页 17；OCR L711
  sentence_summary: Mahon 1947 第 157–191 页被引作学院研究参考。
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/01_CHP-1.md
    source_span: lines 711; print pp. 17
  original_quotes:
  - source_span: "lines 711–711"
    text: |2-
      4 Pevsner, p. 64, and Mahon, 1947, pp. 157-91.
- citation: "Wikipedia (en), Denis Mahon, revision 1364120001. https://en.wikipedia.org/wiki/Denis_Mahon. Accessed 2026-09-10."
  location: "导言身份段；REV-034 初步对齐，非全文补足"
  sentence_summary: "John Denis Mahon，英国意大利艺术史研究者及收藏家；不把历史持有延伸为今天收藏。"
- citation: "Wikidata, Q3705445, revision 2536263887. https://www.wikidata.org/wiki/Q3705445. Accessed 2026-09-10."
  location: "labels／descriptions／P31／适用身份字段及 enwiki sitelink；判断范围见正文"
  sentence_summary: "John Denis Mahon，英国意大利艺术史研究者及收藏家；不把历史持有延伸为今天收藏。"
- citation: "Wikipedia (en), Denis Mahon, revision 1364120001. https://en.wikipedia.org/wiki/Denis_Mahon Accessed 2026-09-11."
  location: "全文7289字符；章节：Biography、Awards、Legacy、References、Further reading、External links"
  sentence_summary: "完成身份复核与全文语义阅读；只把与本卡类型和第一章语境相关的内容写入结构字段。"
- citation: "Wikidata, Q3705445, revision 2536263887. https://www.wikidata.org/wiki/Q3705445. Accessed 2026-09-11."
  location: "实体完整抓取；含rank、qualifiers、references及enwiki sitelink"
  sentence_summary: "与Wikipedia双向身份一致；结构字段保留参考状态，未机械接收全部声明。"
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-persons-denis-mahon
---

## 内容

### 描述

**中文：** Haskell 写作时收藏一幅 Sacchi 为罗马 Capuchin 教堂所作祭坛稿；章中另以 Mahon 1947 为学院研究引用。馆藏状态不延伸到今天。

**English:** Mahon owned a Sacchi Capuchin altarpiece modello at the time of Haskell's account. Mahon 1947 is separately cited on the Academy. The historical holding is not extended to today.

### 已有信息

| 项目 | 内容 | 依据 |
|---|---|---|
| 名称／对应英文 | Denis Mahon | S1、S2 |
| 收藏与研究线索 | Sacchi 祭坛稿；Mahon 1947 学院研究 | S1、S2 |

### 结构化信息

**语义摘要：** 英国艺术史家和收藏家，以意大利巴洛克尤其Guercino研究著称；页面记其收藏、学术倡议和向公共机构捐赠／长期借展。1961年所持Sacchi稿本是特定历史时点。

| 字段 | 当前值 | 依据／状态 |
|---|---|---|
| 性别 | 男 | Wikidata P21；有参考 |
| 出生日期 | 1910-11-08 | Wikidata P569；有参考 |
| 去世日期 | 2011-04-24 | Wikidata P570；有参考 |
| 出生地 | 倫敦 | Wikidata P19；有参考 |
| 去世地 | 卡多根广场 | Wikidata P20；有参考 |
| 国籍／公民身份 | 英国 | Wikidata P27；有参考 |
| 身份／职业标签 | 美術史學家；艺术收藏家 | Wikidata P106；有参考 |
| 父亲 | John FitzGerald Mahon | Wikidata P22；有参考 |
| 母亲 | Lady Alice Browne | Wikidata P25；有参考 |
| 教育／训练机构 | 牛津大学基督堂学院；科陶德藝術學院；伊顿公学 | Wikidata P69；有参考；未列参考 |
| 荣誉／奖项 | 大英帝國司令勳章；英国科学院院士；下級勳位爵士；Companion of Honour | Wikidata P166；有参考；未列参考 |

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 收藏者／所有者（`owner_of`，反向投影） | [萨基罗马嘉布遣会教堂祭坛稿（Sacchi’s modello for a Roman Capuchin church altarpiece）](../works/sacchi-capuchin-altarpiece-modello.md) | 本书记录时属于 Mahon 收藏，不延伸到今天；角色：本书记录时的收藏者；范围：历史收藏状态；不延伸到今天；原断言与证据见发出端卡片“萨基罗马嘉布遣会教堂祭坛稿（Sacchi’s modello for a Roman Capuchin church altarpiece）”：[来源](../../../02-sources/02-Markdown/01_CHP-1.md)；patrons-and-painters；lines 467–468; print pp. 12 |

### 身份与外部链接

- [Q3705445](https://www.wikidata.org/wiki/Q3705445)
- [Wikipedia（en）](https://en.wikipedia.org/wiki/Denis_Mahon)
