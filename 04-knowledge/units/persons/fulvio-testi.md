---
title: "富尔维奥·泰斯蒂（Fulvio Testi）"
name_en: "Fulvio Testi"
type: person
created: 2026-09-09
updated: '2026-09-25'
evidence_status: source_backed
sources:
  - citation: "Francis Haskell, Patrons and Painters (New Haven and London: Yale University Press, 1980), ch. 1, pp. 17."
    location: "第一章印刷页 17；OCR 行 700–703"
    sentence_summary: "转述：Haskell 引泰斯蒂致丰塔纳伯爵的信，记贝尔尼尼雕像估价、博尔盖塞枢机肖像酬金及圣彼得大教堂月俸。"
    evidence_ref:
      doc_id: patrons-and-painters
      source_file: 02-sources/02-Markdown/01_CHP-1.md
      source_span: "lines 700–703; print pp. 17"
    original_quotes:
    - source_span: "lines 700–703"
      text: |2-
        nucci, 1948, p. 89. A letter from Fulvio Testi to Conte Francesco Fontana published by Fraschetti (p. 108)
        shows the_prices that Bernini could command in 1633—a statue would be worth 4000 or 5000 scudi;
        he was paid 1000 scudi for the head of Cardinal Borghese; he received a regular salary of 300 scudi a month
        from the Fabbrica di S. Pietro.
  - citation: '"Fulvio Testi," English Wikipedia, revision 1372649729. https://en.wikipedia.org/wiki/Fulvio_Testi. Accessed 2026-09-09.'
    location: "信息框、Life、Main works、References、Bibliography；对象页全文已读，未通读全部外链"
    sentence_summary: "转述：页面介绍泰斯蒂的外交与诗歌活动、家庭、作品和后世评价；部分日期与 Treccani 有差异，未照单接收。"
  - citation: 'Wikidata, "Fulvio Testi," Q594614, revision 2520426248. https://www.wikidata.org/wiki/Q594614. Accessed 2026-09-09.'
    location: "Statements、Identifiers、enwiki sitelink；双向配对通过后提取适用字段"
    sentence_summary: "属性摘要：职业为外交官、诗人、政治人物、作家；另列生卒地、日期、学习机构和会员身份，日期与无引用值逐项辨析。"
  - citation: 'Marco Leone, "TESTI, Fulvio," Dizionario Biografico degli Italiani 95 (2019). https://www.treccani.it/enciclopedia/fulvio-testi_(Dizionario-Biografico)/. Accessed 2026-09-09.'
    location: "署名传记正文及 Fonti e Bibl.；网页无印刷页码，已读全文"
    sentence_summary: "转述：首段把 1593-08-23 明确为受洗日期；传记说明家庭、教育、埃斯特宫廷任职及外交经历，并讨论诗歌与书信。"
  - citation: 'Stanislao Fraschetti, Il Bernini: la sua vita, la sua opera, il suo tempo (Milano: Ulrico Hoepli, 1900), ch. 12, p. 108, n. 1. https://resources.warburg.sas.ac.uk/pdf/cnd2275b2242323.pdf. Accessed 2026-09-09.'
    location: "第十二章，印刷页 108 注 1（PDF 第 158 页）；刊引文字及扫描图已读"
    sentence_summary: "转述：泰斯蒂在 1633-01-29 罗马来信中称贝尔尼尼参加其交谈圈、赠送素描并已开始为其画肖像；这是作者自述和评价。"
  - citation: "Wikipedia (en), Fulvio Testi, revision 1372649729. https://en.wikipedia.org/wiki/Fulvio_Testi. Accessed 2026-09-10."
    location: "导言身份段；REV-034 初步对齐，非全文补足"
    sentence_summary: "复用并复核既有诗人／埃斯特宫廷官员身份；1593–1646。受洗日与生日仍区分。"
  - citation: "Wikidata, Q594614, revision 2520426248. https://www.wikidata.org/wiki/Q594614. Accessed 2026-09-10."
    location: "labels／descriptions／P31／适用身份字段及 enwiki sitelink；判断范围见正文"
    sentence_summary: "复用并复核既有诗人／埃斯特宫廷官员身份；1593–1646。受洗日与生日仍区分。"
  - citation: "Banca Dati 'Giulio Rospigliosi', letter by Fulvio Testi to Count Francesco Fontana, 1634-01-18, transcribed from Testi, Lettere, ed. Maria Luisa Doglio, vol. II (1967), pp. 28–29. https://www.nuovorinascimento.org/rosp-2000/documenti/testi-34-01-18.htm. Accessed 2026-09-25."
    location: "书信题头、署名及署期"
    sentence_summary: "独立于第一章所引1633年书信，支持Testi与Count Francesco Fontana于1634年继续通信；该单封信仅证明本次通信，不由此推断友谊或赞助。"
    evidence_ref:
      doc_id: testi-fontana-letter-1634-01-18
      source_file: https://www.nuovorinascimento.org/rosp-2000/documenti/testi-34-01-18.htm
      source_span: lines 2–15; edition pp. 28–29
relations:
- relation_type: corresponded_with
  target: persons/francesco-fontana.md
  evidence_ref:
    doc_id: testi-fontana-letter-1634-01-18
    source_file: https://www.nuovorinascimento.org/rosp-2000/documenti/testi-34-01-18.htm
    source_span: lines 2–15; edition pp. 28–29
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 写信人↔收信人
  time: '1634-01-18'
  scope: 仅记录有日期的一次通信；另有1633-01-29书信见对应书信KU；不据通信推断友谊或赞助
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#francesco-fontana身份对齐与1634年通信补足2026-09-25
---

## 内容

### 描述

**中文：** 富尔维奥·泰斯蒂（1593–1646）是服务于埃斯特宫廷的诗人、外交官和宫廷官员，也从事教育及文书工作。第一章引用他致弗朗切斯科·丰塔纳伯爵的信讨论贝尔尼尼的报酬；刊引信件还显示他与贝尔尼尼的交往及对画家、雕塑家才能的评价。各类角色并存，不将其仅归为诗人，也不因接受作品礼物就称其为艺术赞助人。

**English:** Fulvio Testi (1593–1646) was a poet, diplomat and court official in Este service, with additional educational and documentary duties. Chapter 1 uses his letter to Count Francesco Fontana to discuss Bernini's remuneration. The published letter also records his dealings with Bernini and his appraisal of Bernini's abilities as a painter and sculptor. These roles coexist; receiving gifts of art alone does not establish artistic patronage.

### 名称、生平与 Wikidata 字段

| 字段 | 内容及采用状态 | 依据 |
|---|---|---|
| 姓名／别名 | Fulvio Testi；WD 另名 Fulvio Lodovico Testi、Count Fulvio Testi；扩展名保留来源，尚未把无直接原档支持的形式提升为唯一全名 | WP、WD labels；WD 梵蒂冈目录标识的 subject named as |
| 笔名 P742 | Fulvio Savojano，作为有争议作品刊本的署名线索，不证明所有署该名文本均出自本人 | WD 无引用；WP Main works |
| 出生 P569／受洗 | 出生于 1593 年；WD P569 列 1593-08-23，WP 仅精确到月；采用 Treccani 将 8 月 23 日单记受洗日，出生具体日待核 | WD 引 BnF、SNAC；Treccani 首段 |
| 死亡 P570 | 1646-08-28 | WD、WP、Treccani 末段；不把死亡原因自动转成医学结论 |
| 出生地 P19／去世地 P20 | 费拉拉（Ferrara）／摩德纳（Modena） | WD 引 Treccani；该传记正文已核 |
| 语言 P1412 | 意大利语；WP 另列拉丁语，早期拉丁诗的记录支持其写作使用 | WD 引 BnF、CONOR；WP、Treccani 学习段 |
| 就读 P69／师承 P1066 | 博洛尼亚大学、费拉拉大学；Claudio Achillini | WD P69 引 Treccani 旧百科，P1066 无引用；2019 传记明确大学教师关系，不凭“影响”制造师承 |
| 会员 P463 | Ss. Maurizio e Lazzaro、Santiago；Accademia dei Gelati 的起始限定为 1642 | WD 均无引用，Treccani 相应任职／荣衔段可补证；荣衔不等于职业 |
| 权威标识线索 | VIAF 73902706；BnF 12210615p；SBN RAVV037267 | 已读取 WD 值；这些目录目标页尚未逐个核验，不称三个独立身份确认 |

### 多值角色、亲缘与经历

| 属性／时期 | 内容 | 依据与限定 |
|---|---|---|
| 角色标签 | 诗人（poet）、外交官（diplomat）、政治人物／宫廷官员（politician / court official）、作家（writer） | WD P106 多值；具体职责按下列记录区分 |
| 教育角色 | 导师（tutor），对象为 Francesco I d’Este | Treccani“precettore e consigliere”段；不扩大为现代教育机构任职 |
| 文书角色 | 秘书（secretary）、图书馆员（librarian）、档案管理员（archivist） | Treccani 1619 后的宫廷任职段 |
| 贵族身份 | 伯爵（count），WP 记 1635 年获封；作为待原授予文书核对的身份属性 | WP Life；WD 别名包含 Count，但别名本身不能证明授予日期 |
| 家庭 | 父 Giulio，母 Margherita Calmoni；配偶 Anna Leni，1614 年结婚；七名子女 | WP；Treccani 家庭段，子女完整名单尚未逐一核实 |
| 1612 起／1632–1634 | 进入宫廷文书工作；后为驻罗马使节，具体任期据传记为 1632 年 11 月至 1634 年 9 月，间有离任旅行 | Treccani 宫廷与外交段 |
| 1640–1642 | Garfagnana 总督，传记具体记 1640 年 11 月至 1642 年 2 月 | WP Life、Treccani 后半段 |
| 1633-01-29 | 从罗马致丰塔纳伯爵信，记艺术交往和报酬 | [信件知识元](../archives/testi-fontana-bernini-letter.md)；Fraschetti p. 108 注 1 |
| 1634-01-18 | 从罗马致Count Francesco Fontana信，询请代订长枪用groppelle | [信件知识元](../archives/testi-fontana-letter-1634-01-18.md)；Testi, *Lettere*, vol. II, pp. 28–29 |

### 著述、评价与研究线索

| 对象 | 记录及阅读状态 |
|---|---|
| 《诗集》（Rime）／《抒情诗》（Poesie liriche） | WP 列 1617、1627 刊本；Treccani 另记 Rime 的 1613 初刊。保留不同版本，原书未读；中文为描述性暂译 |
| 《书信集》（Lettere） | Maria Luisa Doglio 编，Bari: Laterza, 1967，三卷，按 1609–1633、1634–1637、1638–1646 分卷；WP、Treccani 书目所见，未读全书 |
| 本章对贝尔尼尼的评价 | 信中赞扬其绘画、雕塑与文学谈吐，并记赠画与肖像制作；这是泰斯蒂的署名赞誉，非本项目客观排名。Fraschetti p. 108 注 1 已读 |
| Giacomo Leopardi 对泰斯蒂的评价 | WP 导言转引其高度评价，原信集尚未读；保留评价主体，不作诗史地位的无条件定论 |
| Maria Luisa Doglio，1964，书信研究 | “Intorno alle lettere edite e inedite di Fulvio Testi,” Lettere Italiane 16(4): 425–444；WP／Treccani 书目相符，论文未读 |

这里是已读来源覆盖的属性及书目，不是作品全集。关于《反西班牙檄文》的作者归属，WP 清单与 Treccani 对 Tassoni 归属问题的讨论不能合并为无争议的泰斯蒂作品。

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 通信（`corresponded_with`） | [弗朗切斯科·丰塔纳（Francesco Fontana）](francesco-fontana.md) | 时间：1634-01-18；角色：写信人↔收信人；范围：仅记录有日期的一次通信；另有1633-01-29书信见对应书信KU；不据通信推断友谊或赞助；证据：[来源](https://www.nuovorinascimento.org/rosp-2000/documenti/testi-34-01-18.htm)；lines 2–15; edition pp. 28–29 |
| ← 所著文献（`author_of`，反向投影） | [富尔维奥·泰斯蒂致弗朗切斯科·丰塔纳的信（Letter from Fulvio Testi to Francesco Fontana）](../archives/testi-fontana-bernini-letter.md) | 本章明确书信作者；原断言与证据见发出端卡片“富尔维奥·泰斯蒂致弗朗切斯科·丰塔纳的信（Letter from Fulvio Testi to Francesco Fontana）”：[来源](../../../02-sources/02-Markdown/01_CHP-1.md)；haskell-1980-rev-ed；lines 699–703; print pp. 17 |
| ← 所著文献（`author_of`，反向投影） | [富尔维奥·泰斯蒂致弗朗切斯科·丰塔纳伯爵的信（Letter from Fulvio Testi to Count Francesco Fontana）](../archives/testi-fontana-letter-1634-01-18.md) | 时间：1634-01-18；角色：发信人／作者；原断言与证据见发出端卡片“富尔维奥·泰斯蒂致弗朗切斯科·丰塔纳伯爵的信（Letter from Fulvio Testi to Count Francesco Fontana）”：[来源](https://www.nuovorinascimento.org/rosp-2000/documenti/testi-34-01-18.htm)；lines 8–15; signed by Don Fulvio Testi |

### 身份与外部链接

- [Wikipedia（en）](https://en.wikipedia.org/wiki/Fulvio_Testi)
- [Wikidata Q594614](https://www.wikidata.org/wiki/Q594614)
