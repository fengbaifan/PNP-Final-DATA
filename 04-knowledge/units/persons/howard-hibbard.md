---
title: 霍华德·希巴德（Howard Hibbard）
name_en: Howard Hibbard
type: person
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：第二版序言；印刷页vi；PDF 4；OCR L12–12
  sentence_summary: 哈斯克尔感谢他指出书中的错误。
  original_quotes:
  - source_span: lines 12–12
    text: Mrs Norah Smallwood) for the constant support and encouragement I received from them when we worked together
      on the first handsome edition of this book. And I would also like to thank John Nicoll for proposing a new
      edition and for giving me so much help over it. Robert Enggass, Howard Hibbard and especially Anthony Blunt
      stand out among the many people who have pointed out to me some of my more glaring mistakes, and I am most
      grateful to them as well as to Bruce Boucher, Marco
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_02_Preface_2nd_Ed.md
    source_span: lines 12–12; 章前：第二版序言；印刷页vi；PDF 4
- citation: American Academy of Arts and Sciences, Howard Hibbard, member record. https://www.amacad.org/person/howard-hibbard.
    Accessed 2026-09-14.
  location: 完整人物字段：姓名、生卒、机构、学科、当选年；页面更新2025-04
  sentence_summary: Howard Hibbard，1928–1984，艺术史学家和教育者，列Columbia University及1969当选；用于与序言中的艺术史学者身份配对。
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#作者与序言学者身份
relations:
- relation_type: member_of
  target: institutions/american-academy-of-arts-and-sciences.md
  evidence_ref:
    doc_id: persons-howard-hibbard
    source_file: https://www.amacad.org/person/howard-hibbard
    source_span: 完整人物字段：姓名、生卒、机构、学科、当选年；页面更新2025-04
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 成员目录Elected1969；不据此指定Columbia任期
  role: 当选成员
  time: '1969'
---

## 内容

### 描述

**中文：** 艺术史学家和教育者，本书第二版序言感谢他指出错误。

**English:** An art historian and educator thanked in the second-edition preface for identifying errors.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 惯用名 | Howard Hibbard | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 作品与活动记录

| 字段 | 值 | 证据 |
|---|---|---|
| 章前记载 | 哈斯克尔感谢他指出书中的错误。 | S1 |

### 基本信息

| 字段 | 值 | 证据 |
|---|---|---|
| 生年 | 1928 | S2 |
| 卒年 | 1984 | S2 |
| 身份 | 艺术史学家／Art historian | S2 |
| 身份 | 教育者／Educator | S2 |

### 学术成员身份

| 字段 | 值 | 证据 |
|---|---|---|
| 1969年当选 | [美国艺术与科学院](../institutions/american-academy-of-arts-and-sciences.md)成员；Visual Arts领域 | S2 |


## 关系与证据

### 身份与外部链接

[American Academy of Arts and Sciences人物记录](https://www.amacad.org/person/howard-hibbard)（S2，姓名与学术身份）。

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 隶属／任职于（`member_of`） | [美国艺术与科学院（American Academy of Arts and Sciences）](../institutions/american-academy-of-arts-and-sciences.md) | 时间：1969；角色：当选成员；范围：成员目录Elected1969；不据此指定Columbia任期；证据：[来源](https://www.amacad.org/person/howard-hibbard)；src-0fd5028d6ce7d489；完整人物字段：姓名、生卒、机构、学科、当选年；页面更新2025-04 |
| ← 贡献者（`contributor_to`，反向投影） | [《赞助人与画家》（Patrons and Painters）](../archives/patrons-and-painters.md) | 角色：纠错建议；范围：第二版序言感谢其指出书中错误；原断言与证据见发出端卡片“《赞助人与画家》（Patrons and Painters）”：[来源](../../../02-sources/02-Markdown/00_02_Preface_2nd_Ed.md)；haskell-1980-rev-ed；lines 12–12; 章前：第二版序言；印刷页vi；PDF 4 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
