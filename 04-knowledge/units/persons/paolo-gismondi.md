---
title: 保罗·吉斯蒙迪（Paolo Gismondi）
name_en: Paolo Gismondi
type: person
evidence_status: source_backed
sources:
- citation: Bibliothèque nationale de France, La galerie Mazarine. https://www.bnf.fr/en/mazarin-gallery. Accessed 2026-09-14.
  location: 完整正文、历史沿革及图注；尤其La galerie d’apparat du palais Mazarin
  sentence_summary: 1644年Mazarin委托François Mansart扩建上下画廊；上层穹顶1646–1647年由Giovanni Francesco Romanelli及作坊绘制，列Paolo Gismondi，镀金灰泥责任者Ottaviano Ottoviani。页面中的未来修复计划不作当前状态。
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#马扎然画廊与版画责任
---

## 内容

### 描述

**中文：** 参与罗马内利作坊在马扎然画廊穹顶绘画的艺术家。

**English:** An artist who participated in Romanelli’s workshop painting of the vault of the Galerie Mazarine.

### 姓名

| 字段 | 值 | 证据 |
|---|---|---|
| 全名 | Paolo Gismondi／保罗·吉斯蒙迪 | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 作品与参与

| 字段 | 值 | 证据 |
|---|---|---|
| 1646–1647年参与绘画 | [马扎然画廊穹顶装饰](../works/mazarin-gallery-vault-decoration.md) | S1 |
| 作坊主持画家 | [乔万尼·弗朗切斯科·罗马内利](romanelli.md) | S1 |

## 关系与证据

### 身份与外部链接

[BnF画廊作坊记录](https://www.bnf.fr/en/mazarin-gallery)（S1，具名参与者）。

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 所创作对象（`creator_of`，反向投影） | [马扎然画廊穹顶装饰（Vault Decoration of the Galerie Mazarine）](../works/mazarin-gallery-vault-decoration.md) | 时间：1646–1647绘画工程；角色：作坊绘画参与者；范围：上层画廊装饰整体的分工；1646–1647仅明确用于壁画，未细化灰泥完工日期；原断言与证据见发出端卡片“马扎然画廊穹顶装饰（Vault Decoration of the Galerie Mazarine）”：[来源](https://www.bnf.fr/en/mazarin-gallery)；src-74f81a2927eecba5；La galerie d’apparat du palais Mazarin：委托、绘制者、灰泥与上层穹顶 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
