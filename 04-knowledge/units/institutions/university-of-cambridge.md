---
title: 剑桥大学（University of Cambridge）
name_en: University of Cambridge
type: institution
evidence_status: source_backed
sources:
- citation: Cambridge University Library, A Journey Around the World Mind (2005), ISBN 0-902205-60-9. https://api.repository.cam.ac.uk/server/api/core/bitstreams/678848c7-a7b2-4a76-a937-ea1bbd5ba59c/content.
    Accessed 2026-09-14.
  location: 题名页PDF2、馆长导言PDF3／印刷1；年表和出版项PDF38／印刷36，年表页图像已查看
  sentence_summary: 剑桥大学图书馆名称、研究服务与材料收藏；1416是最早明确提及图书馆的记录年，1424为首份目录年，1934迁馆；2005出版项列West Road地址。
- citation: University of Cambridge, About the University. https://www.cam.ac.uk/about-the-university. Accessed
    2026-09-15.
  location: 完整About正文：History、Mission与各简介卡；未展开子页面
  sentence_summary: 校方将学术历史追溯至1209年，使命为教育、学习和研究。
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#大学博物馆与馆藏关系集中补足
relations:
- relation_type: located_at
  target: places/cambridge.md
  evidence_ref:
    doc_id: institutions-university-of-cambridge
    source_file: https://www.cam.ac.uk/about-the-university
    source_span: Civic and community engagement及机构名称
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 主要所在地
  scope: 机构所在城市；不以单一城市概括全部异地教学场所
  time: 来源访问时点2026-09-15
---

## 内容

### 描述

**中文：** 位于英国剑桥的大学，剑桥大学图书馆是其研究图书馆。

**English:** A university in Cambridge, England, whose research library is Cambridge University Library.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 英文全称 | University of Cambridge | S1，印刷36出版项 |

### 机构信息

| 字段 | 值 | 证据 |
|---|---|---|
| 性质 | 大学／University | S1 |
| 所在地 | [剑桥](../places/cambridge.md) | S1 |
| 研究图书馆 | [剑桥大学图书馆](university-library-cambridge.md) | S1，印刷1、36 |

### 机构沿革

| 字段 | 值 | 证据 |
|---|---|---|
| 学术历史起点 | 1209 | S2 |


## 关系与证据

### 身份与外部链接

[校方机构与历史介绍](https://www.cam.ac.uk/about-the-university)（S2）。

[剑桥大学图书馆出版物](https://api.repository.cam.ac.uk/server/api/core/bitstreams/678848c7-a7b2-4a76-a937-ea1bbd5ba59c/content)（S1，大学与图书馆的关联）。

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 包含成员（`contains`，反向投影） | [剑桥大学图书馆（Cambridge University Library）](university-library-cambridge.md) | 角色：所属图书馆；范围：大学所属研究图书馆，不与大学合并为同一KU；原断言与证据见发出端卡片“剑桥大学图书馆（Cambridge University Library）”：[来源](https://api.repository.cam.ac.uk/server/api/core/bitstreams/678848c7-a7b2-4a76-a937-ea1bbd5ba59c/content)；src-7a4785dfb96dd6b8；2005馆长导言PDF3／印刷1及PDF38／印刷36年表 |
| → 位于（`located_at`） | [剑桥（Cambridge）](../places/cambridge.md) | 时间：来源访问时点2026-09-15；角色：主要所在地；范围：机构所在城市；不以单一城市概括全部异地教学场所；证据：[来源](https://www.cam.ac.uk/about-the-university)；src-01153e0b51090d58；Civic and community engagement及机构名称 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)

[英国供片机构候选与裁决](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#英国供片机构与大学图书馆)
