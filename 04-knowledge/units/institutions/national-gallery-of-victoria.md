---
title: 维多利亚国家美术馆（National Gallery of Victoria）
name_en: National Gallery of Victoria
type: institution
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图版目录；印刷页xvi；OCR L144–145
  sentence_summary: 图版61b目录将其记为书中收藏／保管者。
  original_quotes:
  - source_span: lines 144–145
    text: 'b Tiepolo: Banquet of Anthony and Cleopatra (National Gallery of

      Victoria, Felton Bequest, Melbourne)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 144–145; 章前：图版目录；印刷页xvi
- citation: National Gallery of Victoria, The Banquet of Cleopatra, 103-4. https://www.ngv.vic.gov.au/explore/collection/work/4409/. Accessed 2026-09-14.
  location: 作者、作品信息、Accession Number
  sentence_summary: Giambattista Tiepolo；1743–1744；布面油画；250.3×357.0厘米；Felton Bequest 1933；藏品号103-4。用于图版61b身份及版本核对。
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#馆藏机构与图版版本集中核对
---

## 内容

### 描述

**中文：** 墨尔本的艺术收藏机构，保存本书图版所列《克娄巴特拉的宴会：墨尔本藏本》。

**English:** The gallery in Melbourne holding Tiepolo’s Banquet of Cleopatra, listed as plate 61b.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | National Gallery of Victoria | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版61b角色 | 书中收藏／保管者 | S1 |
| 图版61b相关对象 | [安东尼与克娄巴特拉宴会：维多利亚国家美术馆藏本](../works/tiepolo-banquet-victoria.md)；书中收藏／保管者 | S1 |

### 机构与对应藏品

| 字段 | 值 | 证据 |
|---|---|---|
| 所在地 | [墨尔本](../places/melbourne.md) | S1 |
| 馆藏作品 | [克娄巴特拉的宴会：墨尔本藏本](../works/tiepolo-banquet-victoria.md) | S2 |
| 馆藏号 | 103-4 | S2 |

## 关系与证据

### 身份与外部链接

[对应馆藏103-4](https://www.ngv.vic.gov.au/explore/collection/work/4409/)（S2）。

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 保管对象（`holder_of`，反向投影） | [克娄巴特拉的宴会：维多利亚国家美术馆藏本（The Banquet of Cleopatra, National Gallery of Victoria version）](../works/tiepolo-banquet-victoria.md) | 角色：保管机构；范围：103-4馆藏；具体馆内展室随目录时点，不作所有年代定位；原断言与证据见发出端卡片“克娄巴特拉的宴会：维多利亚国家美术馆藏本（The Banquet of Cleopatra, National Gallery of Victoria version）”：[来源](https://www.ngv.vic.gov.au/explore/collection/work/4409/)；src-3e2fa9e50de73317；Credit Line、Gallery location、Accession Number |
| ← 取得者（`acquirer_of`，反向投影） | [克娄巴特拉的宴会：维多利亚国家美术馆藏本（The Banquet of Cleopatra, National Gallery of Victoria version）](../works/tiepolo-banquet-victoria.md) | 时间：1933；角色：取得机构；范围：103-4于1933取得；Felton Bequest为资金／收藏来源署名，不推定Felton本人当年直接赠画；原断言与证据见发出端卡片“克娄巴特拉的宴会：维多利亚国家美术馆藏本（The Banquet of Cleopatra, National Gallery of Victoria version）”：[来源](https://www.ngv.vic.gov.au/explore/collection/work/4409/)；src-3e2fa9e50de73317；Credit Line及Frame：acquired by the National Gallery of Victoria in 1933 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
