---
title: 中央编目与文献研究所（Istituto Centrale per il Catalogo e la Documentazione）
name_en: Istituto Centrale per il Catalogo e la Documentazione
type: institution
evidence_status: source_backed
sources:
- citation: ICCD, Gabinetto Fotografico Nazionale. https://iccd.beniculturali.it/it/fotografia/gabinettofotograficonazionale. Accessed 2026-09-14.
  location: 完整介绍正文，设立与1975年并入ICCD段、档案构成段
  sentence_summary: 1895年设立摄影室，以记录意大利历史、艺术和景观遗产；1975年并入ICCD，摄影活动延续。
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片机构与复制图像角色
---

## 内容

### 描述

**中文：** 意大利文化遗产编目与文献机构，国家摄影室于1975年并入其中。

**English:** An Italian institution for heritage cataloguing and documentation, into which the Gabinetto Fotografico Nazionale was incorporated in 1975.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 意大利文全称 | Istituto Centrale per il Catalogo e la Documentazione | S1 |
| 简称 | ICCD | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 机构与组成

| 字段 | 值 | 证据 |
|---|---|---|
| 业务领域 | 文化遗产编目与文献／Heritage cataloguing and documentation | S1 |
| 组成机构 | [国家摄影室](gabinetto-fotografico-nazionale.md) | S1 |
| 该机构并入年 | 1975 | S1 |

## 关系与证据

### 身份与外部链接

[ICCD国家摄影室介绍](https://iccd.beniculturali.it/it/fotografia/gabinettofotograficonazionale)（S1）。

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 包含成员（`contains`，反向投影） | [国家摄影室（Gabinetto Fotografico Nazionale）](gabinetto-fotografico-nazionale.md) | 时间：1975起；角色：组成机构；范围：1975年并入后作为ICCD组成部分继续运作；不记录为停止摄影活动；原断言与证据见发出端卡片“国家摄影室（Gabinetto Fotografico Nazionale）”：[来源](https://iccd.beniculturali.it/it/fotografia/gabinettofotograficonazionale)；institutions-gabinetto-fotografico-nazionale；机构史1895设立与1975并入段 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)

[供片角色候选与身份裁决](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片机构与复制图像角色)
