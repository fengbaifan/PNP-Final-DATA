---
title: 查托与温达斯出版社（Chatto and Windus）
name_en: Chatto and Windus
type: institution
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：第二版序言；印刷页vi；PDF 4；OCR L11–12
  sentence_summary: 其负责人在本书初版制作中给予作者支持。
  original_quotes:
  - source_span: lines 11–12
    text: 'I am deeply grateful to the directors of Chatto and Windus (and especially to

      Mrs Norah Smallwood) for the constant support and encouragement I received from them when we worked together on the first handsome edition of this book. And I would also like to thank John Nicoll for proposing a new edition and for giving me so much help over it. Robert Enggass, Howard Hibbard and especially Anthony Blunt stand out among the many people who have pointed out to me some of my more glaring mistakes, and I am most grateful to them as well as to Bruce Boucher, Marco'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_02_Preface_2nd_Ed.md
    source_span: lines 11–12; 章前：第二版序言；印刷页vi；PDF 4
- citation: University of Reading Special Collections, Records of Chatto & Windus Ltd, CW, record 110292665. https://www.reading.ac.uk/adlib/Details/archiveSpecial/110292665. Accessed 2026-09-14.
  location: 已读目录Title、Creator History、Scope；后续重访返回超时／500，未读档案原件
  sentence_summary: 记录生产者为Chatto and Windus Limited，1873年成为Chatto & Windus；前身为Hotten书业。档案年代1860s–1990s不作为公司存续起止；Creator History注明参考出版集团历史。
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#出版研究机构与图书馆角色
---

## 内容

### 描述

**中文：** 英国出版企业，1873年采用Chatto & Windus名称；其负责人参与《赞助人与画家》初版的制作并支持作者。

**English:** A British publishing firm that took the name Chatto & Windus in 1873, whose directors supported Haskell in producing the first edition of Patrons and Painters.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Chatto and Windus | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |
| 档案生产者名称 | Chatto and Windus Limited | S2 |
| 商号名 | Chatto & Windus | S2 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 章前记载 | 其负责人在本书初版制作中给予作者支持。 | S1 |

### 机构信息

| 字段 | 值 | 证据 |
|---|---|---|
| 性质 | 出版企业／Publishing firm | S2 |
| 采用Chatto & Windus名称年份 | 1873 | S2 |

### 本书初版制作

| 字段 | 值 | 证据 |
|---|---|---|
| 支持的作者 | [弗朗西斯·哈斯克尔](../persons/francis-haskell.md) | S1 |
| 相关文献 | [《赞助人与画家》](../archives/patrons-and-painters.md)初版 | S1 |
| 被特别致谢的参与者 | [诺拉·斯莫尔伍德](../persons/norah-smallwood.md) | S1 |

## 关系与证据

### 身份与外部链接

[雷丁大学CW档案目录](https://www.reading.ac.uk/adlib/Details/archiveSpecial/110292665)（S2，企业名称与历史；非原件全文）。

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← supports_claim（`supports_claim`，反向投影） | [《赞助人与画家》（Patrons and Painters）](../archives/patrons-and-painters.md) | 角色：初版制作支持；范围：第二版序言回顾公司负责人员给予初版制作的支持；未拆分具体负责人；原断言与证据见发出端卡片“《赞助人与画家》（Patrons and Painters）”：[来源](../../../02-sources/02-Markdown/00_02_Preface_2nd_Ed.md)；patrons-and-painters；lines 11–12; 章前：第二版序言；印刷页vi；PDF 4 |
| ← 雇主（`employer_of`，反向投影） | [彼得·约翰·安布罗斯·卡尔沃科雷西（Peter John Ambrose Calvocoressi）](../persons/peter-calvocoressi.md) | 时间：1955年加入；角色：合伙人，后任董事；范围：出版履历；不倒推为本书文字协助的合同角色；原断言与证据见发出端卡片“彼得·约翰·安布罗斯·卡尔沃科雷西（Peter John Ambrose Calvocoressi）”：[来源](https://docs.google.com/document/d/1XD-r_i6baE533sA46h4Q4w6vhpGSCMgX0-E6-iL_-Do/pub?embedded=true)；persons-peter-calvocoressi；由作品2671实际嵌入的目录全文：生平、流传、展览、文献及注释；未读所引自传 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
