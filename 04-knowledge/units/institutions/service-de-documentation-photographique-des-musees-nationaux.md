---
title: 法国国家博物馆摄影文献服务处（Service de Documentation Photographique des Musées Nationaux）
name_en: Service de Documentation Photographique des Musées Nationaux
type: institution
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图片来源；印刷页xvi；OCR L169–170
  sentence_summary: 本书图片来源列明的供片者。
  original_quotes:
  - source_span: lines 169–170
    text: 'Academy: 3b, 30a, 30b, 31a, 31b, 47, 49b, 51, 54, 61b; Oscar Savio: I, 11a, 11b, 16; Service de Docu­

      mentation Photographique des Musees Nationaux, Paris: 27b; Soprintendenza alle Gallerie, Florence:'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 169–170; 章前：图片来源；印刷页xvi
- citation: Bibliothèque nationale de France, FRBNF45029807, Portraits de la marquise de Pompadour. https://catalogue.bnf.fr/ark:/12148/cb45029807c. Accessed 2026-09-14.
  location: 完整书目记录，Note(s)、日期、署名与保管字段
  sentence_summary: 一张照片署Service de documentation photographique des Musées Nationaux，另两张署Giraudon。1950–1969是照片集合日期，不是供片机构存续期。
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#机构沿革出版责任与研究支持定稿
relations:
- relation_type: located_at
  target: places/paris.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 169–170; 章前：图片来源；印刷页xvi
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 本书供片署名所列巴黎；不推定今日地址
---


## 内容

### 描述

**中文：** 巴黎的国家博物馆摄影文献服务机构，以该机构署名向本书提供图版27b。

**English:** A photographic documentation service of the national museums in Paris, credited as the supplier of the photograph for plate 27b in the book.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Service de Documentation Photographique des Musées Nationaux | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |
| 外部记录署名 | Service de documentation photographique des Musées Nationaux | S2 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 所供图版 | 27b | S1 |
| 供片对应对象 | [路易十四肖像](../works/bernini-louis-xiv.md)；图版27b复制图像 | S1 |

### 机构信息

| 字段 | 值 | 证据 |
|---|---|---|
| 业务 | 博物馆藏品复制摄影／Reproduction photography of museum objects | S2 |
| 原书所列所在地 | [巴黎](../places/paris.md) | S1 |

## 关系与证据

### 身份与外部链接

[BnF照片目录FRBNF45029807](https://catalogue.bnf.fr/ark:/12148/cb45029807c)（S2，摄影署名记录）。

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 位于（`located_at`） | [巴黎（Paris）](../places/paris.md) | 范围：本书供片署名所列巴黎；不推定今日地址；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；haskell-1980-rev-ed；lines 169–170; 章前：图片来源；印刷页xvi |
| ← 供应者（`supplier_of`，反向投影） | [路易十四肖像（Portrait of Louis XIV）](../works/bernini-louis-xiv.md) | 角色：本书复制图像供片者；范围：《赞助人与画家》图版27b的复制图像；不表示作品创作者、所有者或保管者；原断言与证据见发出端卡片“路易十四肖像（Portrait of Louis XIV）”：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；haskell-1980-rev-ed；lines 169–170; 章前：图片来源；印刷页xvi |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)

[供片角色候选与身份裁决](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片机构与复制图像角色)
