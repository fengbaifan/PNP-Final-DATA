---
title: 《洛雷托圣母》（Madonna di Loreto）
name_en: Madonna di Loreto
type: work
created: '2026-09-13'
updated: 2026-09-14
evidence_status: source_backed
relations:
- relation_type: created_by
  target: persons/caravaggio.md
  note: 具体对象页将Madonna di Loreto归于卡拉瓦乔；本边保留版本边界。
  evidence_ref:
    doc_id: created-by-persons-caravaggio-md
    source_file: https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio)
    source_span: 本卡S2；有S4时并参对象字段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 创作者
  scope: 按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。
- relation_type: installed_at
  target: places/sant-agostino-rome.md
  note: 来源将Madonna di Loreto与该实际或历史安置地点对应。
  evidence_ref:
    doc_id: installed-at-places-sant-agostino-rome-md
    source_file: https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio)
    source_span: 本卡S2；有S4时并参对象／地点说明
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: 见作品字段
  role: 安置地点
  scope: 现存
- relation_type: commissioned_by
  target: persons/ermete-cavalletti.md
  note: 来源明确该端点承担Madonna di Loreto的委托角色；不由委托推定当前所有权。
  evidence_ref:
    doc_id: commissioned-by-persons-ermete-cavalletti-md
    source_file: https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio)
    source_span: 本卡S2；有S4时并参委托／历史段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: 约1604–1606
  role: 委托人／委托机构
  scope: 按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。
- relation_type: intended_for
  target: places/sant-agostino-rome.md
  note: Madonna di Loreto原定用于该建筑或礼拜堂。
  evidence_ref:
    doc_id: intended-for-places-sant-agostino-rome-md
    source_file: https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio)
    source_span: 本卡S2；有S4时并参原定安置段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: 约1604–1606
  role: 原定安置地点
  scope: 按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。
sources:
- citation: Wikipedia (en), Caravaggio, revision 1372526492. https://en.wikipedia.org/wiki/Caravaggio. Accessed 2026-09-11.
  location: 人物页具名对象清单第32项及相邻语境；全文REV-052已保存
  sentence_summary: 主页面将Madonna di Loreto列入具名作品、版本或归属讨论；不由此证明全部字段。
- citation: Wikipedia (en), Madonna di Loreto (Caravaggio), revision 1367122647. https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio). Accessed 2026-09-12.
  location: 全文语义阅读；正文、信息框、图注及注释／书目文本；外链页面另计
  sentence_summary: 区分Madonna di Loreto的对象、版本、年代、材料、流传、归属与当前状态。
- citation: Wikidata, Q2713203, https://www.wikidata.org/wiki/Q2713203. Accessed 2026-09-12.
  location: 双向sitelink／pageprops及P31、P170、P571、P195、P276、P127、P186、P217、尺寸、委托与适用引用
  sentence_summary: 仅采用与具体版本相符且不与馆方记录冲突的字段；聚合系列QID与具体实物QID分开。
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-works-caravaggio-madonna-loreto
---

## 内容

### 描述

**中文：** 《洛雷托圣母》，年代为约1604–1606。现存。

**English:** Madonna di Loreto is dated c. 1604–1606. It survives.

### 题名、类型与基本信息

| 字段 | 内容 | 依据 |
|---|---|---|
| 中文规范题名 | 《洛雷托圣母》 | S1–S2 |
| 英文规范题名 | Madonna di Loreto | S2 |
| 作品类型 | 绘画；具体载体与版本见下 | S2–S4（如有） |
| 创作者／归属 | 米开朗基罗·梅里西·达·卡拉瓦乔／Michelangelo Merisi da Caravaggio | S2–S4 |
| 创作年代 | 约1604–1606 | S1–S4 |
| 媒介与材质 | 布面油画（oil on canvas） | S2–S4（如有） |
| 尺寸 | 260 cm × 150 cm (100 in × 59 in) | S2 |
| 当前保管／状态 | 现存；现存 | S2–S4 |
| 馆藏标识 | 未取得可安全采用的单一馆藏号 | S3–S4 |

### 委托、版本、存世与流传

- **对象边界：** 按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。
- **版本／归属状态：** 现存。

### 题材与作品说明

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 创作者（`created_by`） | [卡拉瓦乔（Caravaggio）](../persons/caravaggio.md) | 具体对象页将Madonna di Loreto归于卡拉瓦乔；本边保留版本边界；角色：创作者；范围：按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。；证据：[来源](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))；src-6783b9c6914c6968；本卡S2；有S4时并参对象字段 |
| → 安置于（`installed_at`） | [罗马圣奥古斯丁圣殿（Basilica of Saint Augustine in Campo Marzio）](../places/sant-agostino-rome.md) | 来源将Madonna di Loreto与该实际或历史安置地点对应；时间：见作品字段；角色：安置地点；范围：现存；证据：[来源](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))；src-6783b9c6914c6968；本卡S2；有S4时并参对象／地点说明 |
| → 由其委托（`commissioned_by`） | [埃尔梅特·卡瓦莱蒂（Ermete Cavalletti）](../persons/ermete-cavalletti.md) | 来源明确该端点承担Madonna di Loreto的委托角色；不由委托推定当前所有权；时间：约1604–1606；角色：委托人／委托机构；范围：按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。；证据：[来源](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))；src-6783b9c6914c6968；本卡S2；有S4时并参委托／历史段 |
| → 拟用于（`intended_for`） | [罗马圣奥古斯丁圣殿（Basilica of Saint Augustine in Campo Marzio）](../places/sant-agostino-rome.md) | Madonna di Loreto原定用于该建筑或礼拜堂；时间：约1604–1606；角色：原定安置地点；范围：按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。；证据：[来源](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))；src-6783b9c6914c6968；本卡S2；有S4时并参原定安置段 |

### 身份与外部链接

- [Wikipedia（en）：Madonna di Loreto (Caravaggio)](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))
- [Wikidata Q2713203](https://www.wikidata.org/wiki/Q2713203)
