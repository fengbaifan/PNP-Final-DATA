---
title: 潘与绪任克斯（Pan and Syrinx）
name_en: Pan and Syrinx
type: work
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图版目录；印刷页xiv；OCR L108–108
  sentence_summary: 《赞助人与画家》图版38b目录所记的《潘与绪任克斯》艺术对象。
  original_quotes:
  - source_span: lines 108–108
    text: 'b Sebastiano Ricci: Pan and Syrinx (Palazzo Pitti, Florence)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 108–108; 章前：图版目录；印刷页xiv
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图片来源；印刷页xvi；OCR L170–171
  sentence_summary: 本书图片来源将图版38b复制图像的供片署名归于佛罗伦萨美术馆监管机构。
  original_quotes:
  - source_span: lines 170–171
    text: 'mentation Photographique des Musees Nationaux, Paris: 27b; Soprintendenza alle Gallerie, Florence:

      38a, 38b, 39, 40a; Steam and Son, Cambridge: 17b, 26a, 28b, 55, 56, 63; Turners of Cambridge: 3a, 17c,'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 170–171; 章前：图片来源；印刷页xvi
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片关系集中定稿
relations:
- relation_type: supplied_by
  target: institutions/soprintendenza-alle-gallerie-florence.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 170–171; 章前：图片来源；印刷页xvi
  relation_source: explicit
  review_status: evidence_backed_relation
  bidirectional_required: false
  role: 本书复制图像供片者
  scope: 《赞助人与画家》图版38b的复制图像；不表示作品创作者、所有者或保管者
---


## 内容

### 描述

**中文：** 《赞助人与画家》图版38b目录所记的《潘与绪任克斯》艺术对象。

**English:** The art object titled Pan and Syrinx, listed as plate 38b in Patrons and Painters.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Pan and Syrinx | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版编号 | 38b | S1 |
| 创作者 | [塞巴斯蒂亚诺·里奇](../persons/sebastiano-ricci.md) | S1 |
| 书中位置 | [皮蒂宫](../places/palazzo-pitti.md) | S1 |
| 书中位置 | [佛罗伦萨](../places/florence.md) | S1 |
| 图像题材 | Pan、Syrinx | S1 |

### 复制图像来源

| 字段 | 值 | 证据 |
|---|---|---|
| 本书图版38b供片者 | [佛罗伦萨美术馆监管机构](../institutions/soprintendenza-alle-gallerie-florence.md) | S2 |

## 关系与证据

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 由其供应（`supplied_by`） | [佛罗伦萨美术馆监管机构（Soprintendenza alle Gallerie, Florence）](../institutions/soprintendenza-alle-gallerie-florence.md) | 角色：本书复制图像供片者；范围：《赞助人与画家》图版38b的复制图像；不表示作品创作者、所有者或保管者；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；haskell-1980-rev-ed；lines 170–171; 章前：图片来源；印刷页xvi |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)

[供片角色候选与身份裁决](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片机构与复制图像角色)
