---
title: 《圣乌尔苏拉殉难》（The Martyrdom of Saint Ursula）
name_en: The Martyrdom of Saint Ursula
type: work
created: '2026-09-13'
updated: 2026-09-14
evidence_status: source_backed
relations:
- relation_type: created_by
  target: persons/caravaggio.md
  note: 具体对象页将The Martyrdom of Saint Ursula归于卡拉瓦乔；本边保留版本边界。
  evidence_ref:
    doc_id: created-by-persons-caravaggio-md
    source_file: https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/
    source_span: 本卡S2；有S4时并参对象字段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 创作者
  scope: 页面及馆方将其列为卡拉瓦乔最后阶段作品；自画像解释保留为图像学判断。
- relation_type: held_by
  target: institutions/gallerie-ditalia.md
  note: 当前对象记录将The Martyrdom of Saint Ursula列由该机构保管；保管不自动等于产权。
  evidence_ref:
    doc_id: held-by-institutions-gallerie-ditalia-md
    source_file: https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/
    source_span: 本卡S2；有S4时并参对象字段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: 本轮来源访问时
  role: 保管机构
  scope: 现存
- relation_type: commissioned_by
  target: persons/marcantonio-i-doria.md
  note: 来源明确该端点承担The Martyrdom of Saint Ursula的委托角色；不由委托推定当前所有权。
  evidence_ref:
    doc_id: commissioned-by-persons-marcantonio-i-doria-md
    source_file: https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/
    source_span: 本卡S2；有S4时并参委托／历史段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: '1610'
  role: 委托人／委托机构
  scope: 页面及馆方将其列为卡拉瓦乔最后阶段作品；自画像解释保留为图像学判断。
sources:
- citation: Wikipedia (en), Caravaggio, revision 1372526492. https://en.wikipedia.org/wiki/Caravaggio. Accessed 2026-09-11.
  location: 人物页具名对象清单第48项及相邻语境；全文REV-052已保存
  sentence_summary: 主页面将The Martyrdom of Saint Ursula列入具名作品、版本或归属讨论；不由此证明全部字段。
- citation: Wikipedia (en), The Martyrdom of Saint Ursula (Caravaggio), revision 1373072947. https://en.wikipedia.org/wiki/The_Martyrdom_of_Saint_Ursula_(Caravaggio). Accessed 2026-09-12.
  location: 全文语义阅读；正文、信息框、图注及注释／书目文本；外链页面另计
  sentence_summary: 区分The Martyrdom of Saint Ursula的对象、版本、年代、材料、流传、归属与当前状态。
- citation: Wikidata, Q1132486, https://www.wikidata.org/wiki/Q1132486. Accessed 2026-09-12.
  location: 双向sitelink／pageprops及P31、P170、P571、P195、P276、P127、P186、P217、尺寸、委托与适用引用
  sentence_summary: 仅采用与具体版本相符且不与馆方记录冲突的字段；聚合系列QID与具体实物QID分开。
- citation: Collection or site authority record. https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/. Accessed 2026-09-12.
  location: 对象字段及与身份、版本、保管、安置或委托有关的说明
  sentence_summary: 为The Martyrdom of Saint Ursula提供具体对象或安置记录；支持范围以正文逐项注明。
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-works-caravaggio-martyrdom-saint-ursula
---

## 内容

### 描述

**中文：** 《圣乌尔苏拉殉难》，年代为1610。现存。

**English:** The Martyrdom of Saint Ursula is dated 1610. It survives.

### 题名、类型与基本信息

| 字段 | 内容 | 依据 |
|---|---|---|
| 中文规范题名 | 《圣乌尔苏拉殉难》 | S1–S2 |
| 英文规范题名 | The Martyrdom of Saint Ursula | S2 |
| 作品类型 | 绘画；具体载体与版本见下 | S2–S4（如有） |
| 创作者／归属 | 米开朗基罗·梅里西·达·卡拉瓦乔／Michelangelo Merisi da Caravaggio | S2–S4 |
| 创作年代 | 1610 | S1–S4 |
| 媒介与材质 | 布面油画（oil on canvas） | S2–S4（如有） |
| 尺寸 | 140.5 cm × 170.5 cm (55.3 in × 67.1 in) | S2 |
| 当前保管／状态 | 意大利画廊／Gallerie d'Italia；现存 | S2–S4 |
| 馆藏标识 | 未取得可安全采用的单一馆藏号 | S3–S4 |

### 委托、版本、存世与流传

- **对象边界：** 页面及馆方将其列为卡拉瓦乔最后阶段作品；自画像解释保留为图像学判断。
- **版本／归属状态：** 现存。

### 题材与作品说明

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 创作者（`created_by`） | [卡拉瓦乔（Caravaggio）](../persons/caravaggio.md) | 具体对象页将The Martyrdom of Saint Ursula归于卡拉瓦乔；本边保留版本边界；角色：创作者；范围：页面及馆方将其列为卡拉瓦乔最后阶段作品；自画像解释保留为图像学判断。；证据：[来源](https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/)；created-by-persons-caravaggio-md；本卡S2；有S4时并参对象字段 |
| → 由其保管（`held_by`） | [意大利画廊（Gallerie d'Italia）](../institutions/gallerie-ditalia.md) | 当前对象记录将The Martyrdom of Saint Ursula列由该机构保管；保管不自动等于产权；时间：本轮来源访问时；角色：保管机构；范围：现存；证据：[来源](https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/)；held-by-institutions-gallerie-ditalia-md；本卡S2；有S4时并参对象字段 |
| → 由其委托（`commissioned_by`） | [马尔坎托尼奥一世·多利亚（Marcantonio I Doria）](../persons/marcantonio-i-doria.md) | 来源明确该端点承担The Martyrdom of Saint Ursula的委托角色；不由委托推定当前所有权；时间：1610；角色：委托人／委托机构；范围：页面及馆方将其列为卡拉瓦乔最后阶段作品；自画像解释保留为图像学判断。；证据：[来源](https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/)；commissioned-by-persons-marcantonio-i-doria-md；本卡S2；有S4时并参委托／历史段 |

### 身份与外部链接

- [Wikipedia（en）：The Martyrdom of Saint Ursula (Caravaggio)](https://en.wikipedia.org/wiki/The_Martyrdom_of_Saint_Ursula_(Caravaggio))
- [Wikidata Q1132486](https://www.wikidata.org/wiki/Q1132486)
- [对象／保管机构记录](https://gallerieditalia.com/en/online-museums/artworks/martyrdom_of_saint_ursula-3023/)
