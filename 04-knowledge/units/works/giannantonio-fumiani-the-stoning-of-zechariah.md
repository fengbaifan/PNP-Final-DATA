---
title: 撒迦利亚被石击（The stoning of Zechariah）
name_en: The stoning of Zechariah
type: work
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图版目录；印刷页xiv；OCR L111–111
  sentence_summary: 《赞助人与画家》图版40b目录所记的《撒迦利亚被石击》艺术对象。
  original_quotes:
  - source_span: lines 111–111
    text: 'b GiannAntonio Fumiani: The stoning of Zechariah (Uffizi, Florence)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 111–111; 章前：图版目录；印刷页xiv
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#图版画家与版画家身份角色补足
relations:
- relation_type: created_by
  target: persons/giannantonio-fumiani.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 111–111; 章前：图版目录；印刷页xiv
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 原书对应图版所列制作责任；原设计与刻版分别保留角色
  role: 画家
- relation_type: held_by
  target: institutions/uffizi-gallery.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 111–111; 章前：图版目录；印刷页xiv
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 原书图版目录所记收藏／保管，不宣称2026年现藏状态
  role: 书中保管者
  time: 本书所述时点
- relation_type: located_at
  target: places/florence.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 111–111; 章前：图版目录；印刷页xiv
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 原书目录位置；不外推现藏或画中地点
  role: 书中保管地点
  time: 本书所述时点
---

## 内容

### 描述

**中文：** 《赞助人与画家》图版40b目录所记的《撒迦利亚被石击》艺术对象。

**English:** The art object titled The stoning of Zechariah, listed as plate 40b in Patrons and Painters.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | The stoning of Zechariah | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版编号 | 40b | S1 |
| 创作者 | [乔瓦尼·安东尼奥·富米亚尼](../persons/giannantonio-fumiani.md) | S1 |
| 书中收藏／保管者 | [乌菲齐美术馆](../institutions/uffizi-gallery.md) | S1 |
| 书中位置 | [佛罗伦萨](../places/florence.md) | S1 |
| 图像题材 | Zechariah | S1 |

## 关系与证据

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 创作者（`created_by`） | [乔瓦尼·安东尼奥·富米亚尼（Giovanni Antonio Fumiani）](../persons/giannantonio-fumiani.md) | 角色：画家；范围：原书对应图版所列制作责任；原设计与刻版分别保留角色；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 111–111; 章前：图版目录；印刷页xiv |
| → 由其保管（`held_by`） | [乌菲齐美术馆（Uffizi Gallery）](../institutions/uffizi-gallery.md) | 时间：本书所述时点；角色：书中保管者；范围：原书图版目录所记收藏／保管，不宣称2026年现藏状态；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 111–111; 章前：图版目录；印刷页xiv |
| → 位于（`located_at`） | [佛罗伦萨（Florence）](../places/florence.md) | 时间：本书所述时点；角色：书中保管地点；范围：原书目录位置；不外推现藏或画中地点；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 111–111; 章前：图版目录；印刷页xiv |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
