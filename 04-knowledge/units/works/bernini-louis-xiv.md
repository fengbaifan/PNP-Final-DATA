---
title: 路易十四肖像（Portrait of Louis XIV）
name_en: Portrait of Louis XIV
type: work
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图版目录；印刷页xiii；OCR L80–80
  sentence_summary: 《赞助人与画家》图版27b目录所记的《路易十四》艺术对象。
  original_quotes:
  - source_span: lines 80–80
    text: '27 a Francesco Fanelli: Charles I (Miss Daphne Ionides, London) b Bernini: Louis XIV (Versailles)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 80–80; 章前：图版目录；印刷页xiii
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图片来源；印刷页xvi；OCR L169–170
  sentence_summary: 本书图片来源将图版27b复制图像的供片署名归于国家博物馆摄影文献服务机构。
  original_quotes:
  - source_span: lines 169–170
    text: 'Academy: 3b, 30a, 30b, 31a, 31b, 47, 49b, 51, 54, 61b; Oscar Savio: I, 11a, 11b, 16; Service de Docu­

      mentation Photographique des Musees Nationaux, Paris: 27b; Soprintendenza alle Gallerie, Florence:'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 169–170; 章前：图片来源；印刷页xvi
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片关系集中定稿
relations:
- relation_type: supplied_by
  target: institutions/service-de-documentation-photographique-des-musees-nationaux.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 169–170; 章前：图片来源；印刷页xvi
  relation_source: explicit
  review_status: evidence_backed_relation
  bidirectional_required: false
  role: 本书复制图像供片者
  scope: 《赞助人与画家》图版27b的复制图像；不表示作品创作者、所有者或保管者
---


## 内容

### 描述

**中文：** 《赞助人与画家》图版27b目录所记的《路易十四》艺术对象。

**English:** The art object titled Louis XIV, listed as plate 27b in Patrons and Painters.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Louis XIV | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版编号 | 27b | S1 |
| 创作者 | [贝尔尼尼](../persons/gian-lorenzo-bernini.md) | S1 |
| 描绘／纪念对象 | [路易十四](../persons/louis-xiv.md) | S1 |
| 书中位置 | [凡尔赛](../places/versailles.md) | S1 |

### 复制图像来源

| 字段 | 值 | 证据 |
|---|---|---|
| 本书图版27b供片者 | [国家博物馆摄影文献服务机构](../institutions/service-de-documentation-photographique-des-musees-nationaux.md) | S2 |

## 关系与证据

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 由其供应（`supplied_by`） | [法国国家博物馆摄影文献服务处（Service de Documentation Photographique des Musées Nationaux）](../institutions/service-de-documentation-photographique-des-musees-nationaux.md) | 角色：本书复制图像供片者；范围：《赞助人与画家》图版27b的复制图像；不表示作品创作者、所有者或保管者；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；haskell-1980-rev-ed；lines 169–170; 章前：图片来源；印刷页xvi |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)

[供片角色候选与身份裁决](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#供片机构与复制图像角色)
