---
title: 百合圣母堂立面（Facade of S. Maria del Giglio）
name_en: Facade of S. Maria del Giglio
type: work
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图版目录；印刷页xiv；OCR L112–112
  sentence_summary: 《赞助人与画家》图版41a目录所记的《百合圣母堂立面》艺术对象。
  original_quotes:
  - source_span: lines 112–112
    text: ', 41 a Facade of S. Maria del Giglio, Venice 248'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 112–112; 章前：图版目录；印刷页xiv
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#作品创作收藏与图像角色集中审查
relations:
- relation_type: part_of
  target: places/s-maria-del-giglio.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 112–112; 章前：图版目录；印刷页xiv
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 所属建筑
  scope: 图版41a；立面是教堂建筑构件，作品与整座教堂分别登记
- relation_type: located_at
  target: places/venice.md
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 112–112; 章前：图版目录；印刷页xiv
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 所在城市
  scope: 图版41a；立面是教堂建筑构件，作品与整座教堂分别登记
  time: 本书所述时点
---

## 内容

### 描述

**中文：** 威尼斯百合圣母堂的立面装饰整体，原书列为图版41a。

**English:** The facade ensemble of Santa Maria del Giglio in Venice, listed as plate 41a in the book.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Facade of S. Maria del Giglio | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版编号 | 41a | S1 |
| 所属建筑 | [百合圣母堂](../places/s-maria-del-giglio.md) | S1 |
| 书中位置 | [威尼斯](../places/venice.md) | S1 |

## 关系与证据

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 组成部分（`part_of`） | [百合圣母堂（S. Maria del Giglio）](../places/s-maria-del-giglio.md) | 角色：所属建筑；范围：图版41a；立面是教堂建筑构件，作品与整座教堂分别登记；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；haskell-1980-rev-ed；lines 112–112; 章前：图版目录；印刷页xiv |
| → 位于（`located_at`） | [威尼斯（Venice）](../places/venice.md) | 时间：本书所述时点；角色：所在城市；范围：图版41a；立面是教堂建筑构件，作品与整座教堂分别登记；证据：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；haskell-1980-rev-ed；lines 112–112; 章前：图版目录；印刷页xiv |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
