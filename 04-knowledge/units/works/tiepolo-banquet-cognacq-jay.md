---
title: 克娄巴特拉的宴会：科涅克—杰藏本（The Banquet of Cleopatra, Musée Cognacq-Jay version）
name_en: The Banquet of Cleopatra, Musée Cognacq-Jay version
type: work
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图版目录；印刷页xvi；OCR L142–143
  sentence_summary: 《赞助人与画家》图版61a目录所记的《安东尼与克娄巴特拉宴会》艺术对象。
  original_quotes:
  - source_span: lines 142–143
    text: '61 a Tiepolo: Banquet of Anthony and Cleopatra (Musée Cognacq-Jay, 360

      Paris)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 142–143; 章前：图版目录；印刷页xvi
- citation: Paris Musées, Le Banquet de Cléopâtre, J 104. https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre. Accessed 2026-09-14.
  location: Auteur(s)、Informations détaillées、Commentaire historique
  sentence_summary: Giambattista Tiepolo；1742–1743；50.5×69厘米布面油画；J 104；巴黎馆藏是墨尔本作品的预备版本。
- citation: Paris Musées, Le Banquet de Cléopâtre, J 104. https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre. Accessed 2026-09-15.
  location: 完整对象字段、图像学、历史及取得信息；HTTP200读取
  sentence_summary: J 104为Tiepolo巴黎预备版本，取得方式为遗赠，遗赠者Cognacq, Ernest，取得年1928，保管机构Musée Cognacq-Jay。
- citation: Musée Cognacq-Jay, History of the museum. https://www.museecognacqjay.paris.fr/en/museum/history-museum. Accessed 2026-09-15.
  location: 完整正文及1929年图录序言的网页引文
  sentence_summary: Cognacq在1925年展示私人收藏；1928年去世后巴黎市作为遗赠受益主体推进博物馆，1929-06-04开馆，1990年迁入Hôtel Donon。
created: '2026-09-14'
updated: '2026-09-15'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md#肖像配对与遗赠主体补齐
relations:
- relation_type: created_by
  target: persons/giambattista-tiepolo.md
  evidence_ref:
    doc_id: works-tiepolo-banquet-cognacq-jay
    source_file: https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre
    source_span: Auteur(s)、Date de production、Numéro d’inventaire
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: J 104巴黎预备版本，区别墨尔本藏本
  role: 画家
  time: 1742–1743
- relation_type: held_by
  target: institutions/musee-cognacq-jay.md
  evidence_ref:
    doc_id: works-tiepolo-banquet-cognacq-jay
    source_file: https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre
    source_span: Institution、Numéro d’inventaire
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: J 104保管机构
  role: 保管机构
- relation_type: model_for
  target: works/tiepolo-banquet-victoria.md
  evidence_ref:
    doc_id: works-tiepolo-banquet-cognacq-jay
    source_file: https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre
    source_span: Commentaire historique
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: 馆方明确称巴黎J 104为墨尔本1743–1744作品的modello及version préparatoire；非泛指全部同题作品
  role: 预备版本
  time: 1742–1743
- relation_type: acquired_by
  target: institutions/city-of-paris.md
  evidence_ref:
    doc_id: cognacq-jay-museum-history
    source_file: https://www.museecognacqjay.paris.fr/en/museum/history-museum
    source_span: The museum located Boulevard des Capucines：City of Paris为遗赠受益主体；具体J 104见本卡取得记录
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: J 104随Cognacq收藏于1928年遗赠给巴黎市；Musée Cognacq-Jay为保管馆，1929年开馆
  role: 遗赠受益主体
  time: '1928'
  evidence: J 104遗赠者与1928取得记录：https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre
- relation_type: contributed_by
  target: persons/ernest-cognacq.md
  evidence_ref:
    doc_id: works-tiepolo-banquet-cognacq-jay
    source_file: https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre
    source_span: Nom du donateur, testateur, vendeur、Date d’acquisition
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  scope: J 104具体遗赠者，区别夫妻共同形成收藏的总体表述
  role: 遗赠者
  time: '1928'
---




## 内容

### 描述

**中文：** 提埃坡罗于1742–1743年绘制的布面油画，现藏巴黎科涅克—杰博物馆，编号J 104，是墨尔本《克娄巴特拉的宴会》的预备版本。

**English:** An oil painting on canvas by Tiepolo, dated 1742–1743 and held by Musée Cognacq-Jay in Paris as J 104; a preparatory version for The Banquet of Cleopatra in Melbourne.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Banquet of Anthony and Cleopatra | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版编号 | 61a | S1 |
| 创作者 | [乔万巴蒂斯塔·提埃坡罗（Giambattista Tiepolo）](../persons/giambattista-tiepolo.md) | S1、S2 |
| 书中收藏／保管者 | [科涅克—杰博物馆](../institutions/musee-cognacq-jay.md) | S1 |
| 书中位置 | [巴黎](../places/paris.md) | S1 |
| 描绘对象 | Anthony与Cleopatra／安东尼与克娄巴特拉，具体身份待对齐 | S1 |


### 版本识别

| 字段 | 值 | 证据 |
|---|---|---|
| 馆藏题名 | 克娄巴特拉的宴会／The Banquet of Cleopatra | S2 |
| 创作时间 | 1742–1743 | S2 |
| 材质 | 布面油画／Oil on canvas | S2 |
| 尺寸（高×宽） | 50.5 × 69 cm | S2 |
| 馆藏编号 | J 104 | S2 |
| 版本性质 | [墨尔本藏本](tiepolo-banquet-victoria.md)的预备版本／Preparatory version | S2 |

### 馆藏沿革

| 字段 | 值 | 证据 |
|---|---|---|
| 取得年份 | 1928 | S2 |
| 取得方式 | 遗赠／Bequest | S2 |


### 遗赠双方

| 字段 | 值 | 证据 |
|---|---|---|
| 遗赠者 | [泰奥多尔—埃内斯特·科涅克](../persons/ernest-cognacq.md) | S3 |
| 1928年遗赠受益主体 | [巴黎市政当局](../institutions/city-of-paris.md) | S3、S4 |
| 保管馆 | [科涅克—杰博物馆](../institutions/musee-cognacq-jay.md) | S3 |


## 关系与证据

### 身份与外部链接

| 字段 | 值／链接 | 状态与范围 |
|---|---|---|
| J 104 | [J 104](https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre) | 作品及具体馆藏版本 |


### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 创作者（`created_by`） | [乔万巴蒂斯塔·提埃坡罗（Giambattista Tiepolo）](../persons/giambattista-tiepolo.md) | 时间：1742–1743；角色：画家；范围：J 104巴黎预备版本，区别墨尔本藏本；证据：[来源](https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre)；works-tiepolo-banquet-cognacq-jay；Auteur(s)、Date de production、Numéro d’inventaire |
| → 由其保管（`held_by`） | [科涅克—杰博物馆（Musée Cognacq-Jay）](../institutions/musee-cognacq-jay.md) | 角色：保管机构；范围：J 104保管机构；证据：[来源](https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre)；works-tiepolo-banquet-cognacq-jay；Institution、Numéro d’inventaire |
| → 作为预备模型／样稿（`model_for`） | [克娄巴特拉的宴会：维多利亚国家美术馆藏本（The Banquet of Cleopatra, National Gallery of Victoria version）](tiepolo-banquet-victoria.md) | 时间：1742–1743；角色：预备版本；范围：馆方明确称巴黎J 104为墨尔本1743–1744作品的modello及version préparatoire；非泛指全部同题作品；证据：[来源](https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre)；works-tiepolo-banquet-cognacq-jay；Commentaire historique |
| → 由其取得（`acquired_by`） | [巴黎市政当局（City of Paris）](../institutions/city-of-paris.md) | 时间：1928；角色：遗赠受益主体；范围：J 104随Cognacq收藏于1928年遗赠给巴黎市；Musée Cognacq-Jay为保管馆，1929年开馆；证据：[来源](https://www.museecognacqjay.paris.fr/en/museum/history-museum)；cognacq-jay-museum-history；The museum located Boulevard des Capucines：City of Paris为遗赠受益主体；具体J 104见本卡取得记录 |
| → 由其贡献（`contributed_by`） | [泰奥多尔—埃内斯特·科涅克（Théodore-Ernest Cognacq）](../persons/ernest-cognacq.md) | 时间：1928；角色：遗赠者；范围：J 104具体遗赠者，区别夫妻共同形成收藏的总体表述；证据：[来源](https://www.parismuseescollections.paris.fr/fr/musee-cognacq-jay/oeuvres/le-banquet-de-cleopatre)；works-tiepolo-banquet-cognacq-jay；Nom du donateur, testateur, vendeur、Date d’acquisition |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
