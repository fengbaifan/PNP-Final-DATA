---
title: 《算命者》（卢浮宫第二版）（The Fortune Teller (Louvre second version)）
name_en: The Fortune Teller (Louvre second version)
type: work
created: '2026-09-13'
updated: 2026-09-14
evidence_status: source_backed
relations:
- relation_type: created_by
  target: persons/caravaggio.md
  note: 具体对象页将The Fortune Teller (Louvre second version)归于卡拉瓦乔；本边保留版本边界。
  evidence_ref:
    doc_id: created-by-persons-caravaggio-md
    source_file: https://collections.louvre.fr/ark:/53355/cl010062329
    source_span: 本卡S2；有S4时并参对象字段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 创作者
  scope: 与卡比托利欧第一版分开；Q2610675只用于卢浮宫对象。
- relation_type: held_by
  target: institutions/louvre-museum.md
  note: 当前对象记录将The Fortune Teller (Louvre second version)列由该机构保管；保管不自动等于产权。
  evidence_ref:
    doc_id: held-by-institutions-louvre-museum-md
    source_file: https://collections.louvre.fr/ark:/53355/cl010062329
    source_span: 本卡S2；有S4时并参对象字段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: 本轮来源访问时
  role: 保管机构
  scope: 现存
- relation_type: variant_of
  target: works/caravaggio-fortune-teller-capitoline.md
  note: 本对象与所链对象为同题不同实物、版本或复制关系，不共享QID、尺寸或馆藏。
  evidence_ref:
    doc_id: variant-of-works-caravaggio-fortune-teller-capitoline-md
    source_file: https://en.wikipedia.org/wiki/The_Fortune_Teller_(Caravaggio)
    source_span: 本卡S2；版本段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 同题异版／复制关系
  scope: 与卡比托利欧第一版分开；Q2610675只用于卢浮宫对象。
sources:
- citation: Wikipedia (en), Caravaggio, revision 1372526492. https://en.wikipedia.org/wiki/Caravaggio. Accessed 2026-09-11.
  location: 人物页具名对象清单第8项及相邻语境；全文REV-052已保存
  sentence_summary: 主页面将The Fortune Teller (Louvre second version)列入具名作品、版本或归属讨论；不由此证明全部字段。
- citation: Wikipedia (en), The Fortune Teller (Caravaggio), revision 1368221322. https://en.wikipedia.org/wiki/The_Fortune_Teller_(Caravaggio). Accessed 2026-09-12.
  location: 全文语义阅读；正文、信息框、图注及注释／书目文本；外链页面另计
  sentence_summary: 区分The Fortune Teller (Louvre second version)的对象、版本、年代、材料、流传、归属与当前状态。
- citation: Wikidata, Q2610675, https://www.wikidata.org/wiki/Q2610675. Accessed 2026-09-12.
  location: 双向sitelink／pageprops及P31、P170、P571、P195、P276、P127、P186、P217、尺寸、委托与适用引用
  sentence_summary: 仅采用与具体版本相符且不与馆方记录冲突的字段；聚合系列QID与具体实物QID分开。
- citation: Collection or site authority record. https://collections.louvre.fr/ark:/53355/cl010062329. Accessed 2026-09-12.
  location: 对象字段及与身份、版本、保管、安置或委托有关的说明
  sentence_summary: 为The Fortune Teller (Louvre second version)提供具体对象或安置记录；支持范围以正文逐项注明。
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-works-caravaggio-fortune-teller-louvre
---

## 内容

### 描述

**中文：** 《算命者》（卢浮宫第二版），年代为约1595。现存。

**English:** The Fortune Teller (Louvre second version) is dated c. 1595. It survives.

### 题名、类型与基本信息

| 字段 | 内容 | 依据 |
|---|---|---|
| 中文规范题名 | 《算命者》（卢浮宫第二版） | S1–S2 |
| 英文规范题名 | The Fortune Teller (Louvre second version) | S2 |
| 作品类型 | 绘画；具体载体与版本见下 | S2–S4（如有） |
| 创作者／归属 | 米开朗基罗·梅里西·达·卡拉瓦乔／Michelangelo Merisi da Caravaggio | S2–S4 |
| 创作年代 | 约1595 | S1–S4 |
| 媒介与材质 | 布面油画（oil on canvas） | S2–S4（如有） |
| 尺寸 | 93 cm × 131 cm (37 in × 52 in) | S2 |
| 当前保管／状态 | 卢浮宫博物馆／Louvre Museum；现存 | S2–S4 |
| 馆藏标识 | INV 55；MR 105 | S3–S4 |

### 委托、版本、存世与流传

- **对象边界：** 与卡比托利欧第一版分开；Q2610675只用于卢浮宫对象。
- **版本／归属状态：** 现存。

### 题材与作品说明

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 创作者（`created_by`） | [卡拉瓦乔（Caravaggio）](../persons/caravaggio.md) | 具体对象页将The Fortune Teller (Louvre second version)归于卡拉瓦乔；本边保留版本边界；角色：创作者；范围：与卡比托利欧第一版分开；Q2610675只用于卢浮宫对象。；证据：[来源](https://collections.louvre.fr/ark:/53355/cl010062329)；created-by-persons-caravaggio-md；本卡S2；有S4时并参对象字段 |
| → 由其保管（`held_by`） | [卢浮宫博物馆（Louvre Museum）](../institutions/louvre-museum.md) | 当前对象记录将The Fortune Teller (Louvre second version)列由该机构保管；保管不自动等于产权；时间：本轮来源访问时；角色：保管机构；范围：现存；证据：[来源](https://collections.louvre.fr/ark:/53355/cl010062329)；held-by-institutions-louvre-museum-md；本卡S2；有S4时并参对象字段 |
| → 为其版本／复制（`variant_of`） | [《算命者》（卡比托利欧第一版）（The Fortune Teller (Capitoline first version)）](caravaggio-fortune-teller-capitoline.md) | 本对象与所链对象为同题不同实物、版本或复制关系，不共享QID、尺寸或馆藏；角色：同题异版／复制关系；范围：与卡比托利欧第一版分开；Q2610675只用于卢浮宫对象。；证据：[来源](https://en.wikipedia.org/wiki/The_Fortune_Teller_(Caravaggio))；variant-of-works-caravaggio-fortune-teller-capitoline-md；本卡S2；版本段 |

### 身份与外部链接

- [Wikipedia（en）：The Fortune Teller (Caravaggio)](https://en.wikipedia.org/wiki/The_Fortune_Teller_(Caravaggio))
- [Wikidata Q2610675](https://www.wikidata.org/wiki/Q2610675)
- [对象／保管机构记录](https://collections.louvre.fr/ark:/53355/cl010062329)
