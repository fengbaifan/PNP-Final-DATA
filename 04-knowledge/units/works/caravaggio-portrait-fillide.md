---
title: 《菲利德·梅兰德罗尼肖像》（Portrait of Fillide Melandroni）
name_en: Portrait of Fillide Melandroni
type: work
created: '2026-09-13'
updated: 2026-09-14
evidence_status: source_backed
relations:
- relation_type: created_by
  target: persons/caravaggio.md
  note: 具体对象页将Portrait of Fillide Melandroni归于卡拉瓦乔；本边保留版本边界。
  evidence_ref:
    doc_id: created-by-persons-caravaggio-md
    source_file: https://en.wikipedia.org/wiki/Portrait_of_a_Courtesan_(Caravaggio)
    source_span: 本卡S2；有S4时并参对象字段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  role: 创作者
  scope: 又名《一位交际花的肖像》；与人物菲利德及她作为其他画作模特的推测分开。
- relation_type: owned_by
  target: persons/marchese-giustiniani-ch1.md
  note: 来源支持该端点在所列时期收藏或拥有Portrait of Fillide Melandroni；不等于当前保管。
  evidence_ref:
    doc_id: owned-by-persons-marchese-giustiniani-ch1-md
    source_file: https://en.wikipedia.org/wiki/Portrait_of_a_Courtesan_(Caravaggio)
    source_span: 本卡S2；有S4时并参流传段
  review_status: evidence_backed_relation
  relation_source: explicit
  bidirectional_required: false
  time: 1638年前至1812
  role: 朱斯蒂尼亚尼收藏
  scope: 又名《一位交际花的肖像》；与人物菲利德及她作为其他画作模特的推测分开。
sources:
- citation: Wikipedia (en), Caravaggio, revision 1372526492. https://en.wikipedia.org/wiki/Caravaggio. Accessed 2026-09-11.
  location: 人物页具名对象清单第55项及相邻语境；全文REV-052已保存
  sentence_summary: 主页面将Portrait of Fillide Melandroni列入具名作品、版本或归属讨论；不由此证明全部字段。
- citation: Wikipedia (en), Portrait of a Courtesan (Caravaggio), revision 1361374174. https://en.wikipedia.org/wiki/Portrait_of_a_Courtesan_(Caravaggio). Accessed 2026-09-12.
  location: 全文语义阅读；正文、信息框、图注及注释／书目文本；外链页面另计
  sentence_summary: 区分Portrait of Fillide Melandroni的对象、版本、年代、材料、流传、归属与当前状态。
- citation: Wikidata, Q2395137, https://www.wikidata.org/wiki/Q2395137. Accessed 2026-09-12.
  location: 双向sitelink／pageprops及P31、P170、P571、P195、P276、P127、P186、P217、尺寸、委托与适用引用
  sentence_summary: 仅采用与具体版本相符且不与馆方记录冲突的字段；聚合系列QID与具体实物QID分开。
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-works-caravaggio-portrait-fillide
---

## 内容

### 描述

**中文：** 《菲利德·梅兰德罗尼肖像》，年代为约1598。1945年在柏林毁失或失踪。

**English:** Portrait of Fillide Melandroni is dated c. 1598. It was destroyed or went missing in Berlin in 1945.

### 题名、类型与基本信息

| 字段 | 内容 | 依据 |
|---|---|---|
| 中文规范题名 | 《菲利德·梅兰德罗尼肖像》 | S1–S2 |
| 英文规范题名 | Portrait of Fillide Melandroni | S2 |
| 作品类型 | 绘画；具体载体与版本见下 | S2–S4（如有） |
| 创作者／归属 | 米开朗基罗·梅里西·达·卡拉瓦乔／Michelangelo Merisi da Caravaggio | S2–S4 |
| 创作年代 | 约1598 | S1–S4 |
| 媒介与材质 | 布面油画（oil on canvas） | S2–S4（如有） |
| 尺寸 | 66 cm × 53 cm (26 in × 21 in) | S2 |
| 当前保管／状态 | 1945年在柏林毁失或失踪；1945年在柏林毁失或失踪 | S2–S4 |
| 馆藏标识 | 356 | S3–S4 |

### 委托、版本、存世与流传

- **对象边界：** 又名《一位交际花的肖像》；与人物菲利德及她作为其他画作模特的推测分开。
- **版本／归属状态：** 1945年在柏林毁失或失踪。

### 题材与作品说明

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| → 创作者（`created_by`） | [卡拉瓦乔（Caravaggio）](../persons/caravaggio.md) | 具体对象页将Portrait of Fillide Melandroni归于卡拉瓦乔；本边保留版本边界；角色：创作者；范围：又名《一位交际花的肖像》；与人物菲利德及她作为其他画作模特的推测分开。；证据：[来源](https://en.wikipedia.org/wiki/Portrait_of_a_Courtesan_(Caravaggio))；created-by-persons-caravaggio-md；本卡S2；有S4时并参对象字段 |
| → 由其收藏／拥有（`owned_by`） | [文琴佐·朱斯蒂尼亚尼（Vincenzo Giustiniani）](../persons/marchese-giustiniani-ch1.md) | 来源支持该端点在所列时期收藏或拥有Portrait of Fillide Melandroni；不等于当前保管；时间：1638年前至1812；角色：朱斯蒂尼亚尼收藏；范围：又名《一位交际花的肖像》；与人物菲利德及她作为其他画作模特的推测分开。；证据：[来源](https://en.wikipedia.org/wiki/Portrait_of_a_Courtesan_(Caravaggio))；owned-by-persons-marchese-giustiniani-ch1-md；本卡S2；有S4时并参流传段 |

### 身份与外部链接

- [Wikipedia（en）：Portrait of a Courtesan (Caravaggio)](https://en.wikipedia.org/wiki/Portrait_of_a_Courtesan_(Caravaggio))
- [Wikidata Q2395137](https://www.wikidata.org/wiki/Q2395137)
