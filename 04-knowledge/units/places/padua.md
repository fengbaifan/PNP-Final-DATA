---
title: 帕多瓦（Padua）
name_en: Padua
type: place
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图版目录；印刷页xvi；OCR L146–146
  sentence_summary: 图版62目录将其记为描绘地点／对象。
  original_quotes:
  - source_span: lines 146–146
    text: '62 Canaletto: Prato della Valle, Padua'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 146–146; 章前：图版目录；印刷页xvi
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md
---

## 内容

### 描述

**中文：** 图版62目录将其记为描绘地点／对象。

**English:** The entry for plate 62 records this entity in the role of depicted place or object.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Padua | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版62角色 | 描绘地点／对象 | S1 |
| 图版62相关对象 | [帕多瓦Prà广场](../works/canaletto-pra-della-valle-padua.md)；描绘地点／对象 | S1 |

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 所在地（`location_of`，反向投影） | [帕多瓦大学（University of Padua）](../institutions/university-of-padua.md) | 时间：来源访问时点2026-09-15；角色：主要所在地；范围：机构所在城市；不以单一城市概括全部异地教学场所；原断言与证据见发出端卡片“帕多瓦大学（University of Padua）”：[来源](https://www.unipd.it/en/storia)；institutions-university-of-padua；Contacts: Via 8 Febbraio 2,35122 Padova |
| ← 所在地（`location_of`，反向投影） | [贝尔纳迪诺·卡斯泰利（Bernardino Castelli）](../persons/bernardino-castelli.md) | 时间：1775；角色：迁居；范围：传记明确地点事件；原断言与证据见发出端卡片“贝尔纳迪诺·卡斯泰利（Bernardino Castelli）”：[来源](https://www.treccani.it/enciclopedia/bernardino-castelli_(Dizionario-Biografico)/)；persons-bernardino-castelli；1775年转往Padova段 |
| ← 所在地（`location_of`，反向投影） | [卡洛·洛多利（Carlo Lodoli）](../persons/carlo-lodoli.md) | 时间：1761-10-27；角色：去世；范围：明确生平地点事件，非全生涯连续住址；原断言与证据见发出端卡片“卡洛·洛多利（Carlo Lodoli）”：[来源](https://www.treccani.it/enciclopedia/carlo-lodoli_(Dizionario-Biografico)/)；persons-carlo-lodoli；末段Morì a Padova |
| ← 为其所涉对象（`subject_of`，反向投影） | [帕多瓦Prà广场（Prà della Valle, Padua）](../works/canaletto-pra-della-valle-padua.md) | 角色：描绘城市；范围：图版62标题中的Prato della Valle,Padua；这是描绘地点而非保管地点；原断言与证据见发出端卡片“帕多瓦Prà广场（Prà della Valle, Padua）”：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 146–146; 章前：图版目录；印刷页xvi |
| ← 所在地（`location_of`，反向投影） | [约翰·康拉德·霍夫曼（Johann Conrad Hofmann）](../persons/johann-conrad-hofmann.md) | 时间：1756-03-15；角色：去世；范围：跨页句明确在帕多瓦去世；原断言与证据见发出端卡片“约翰·康拉德·霍夫曼（Johann Conrad Hofmann）”：[来源](https://media.agiati.org/page/attachments/memorie-mem-06-i-buoni-ingegni-della-patria-art02-ferrari.pdf)；persons-amadeo-swajer；印刷pp.52–53跨页句；PDF第2–3页 |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
