---
title: 斯福尔扎城堡（Castello Sforzesco）
name_en: Castello Sforzesco
type: place
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this printing 2006).
  location: 章前：图版目录；印刷页xv；OCR L131–131
  sentence_summary: 图版53a目录将其记为书中位置。
  original_quotes:
  - source_span: lines 131–131
    text: '53 a Piazzetta: Marshal Schulenburg (Castello Sforzesco, Milan) 312 b Amigoni: Sigismund Streit (Gymnasium zu Grauen Kloster, Berlin)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 131–131; 章前：图版目录；印刷页xv
- citation: Regione Lombardia / Gabinetto dei Disegni, SIRBeC 4y010-09081, Ritratto di Matthias von der Schulenburg. https://www.lombardiabeniculturali.it/opere-arte/schede-complete/4y010-09081/. Accessed 2026-09-14.
  location: PDF2–5：对象、馆藏、尺寸、技术、评论与取得；网页同源摘要
  sentence_summary: Piazzetta素描，387×303毫米，库存号4884/5 E 82/5；馆方年代字段约1730–1735，评论另提1738。Schulenburg（1661–1747）任威尼斯共和国军职，也聘Piazzetta估价、评估购藏。
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md
---

## 内容

### 描述

**中文：** 米兰的城堡建筑群，舒伦堡素描的保管部门位于其中。

**English:** A castle complex in Milan housing the collection department that holds the drawing of Schulenburg.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | Castello Sforzesco | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版53a角色 | 书中位置 | S1 |
| 图版53a相关对象 | [舒伦堡元帅肖像](../works/piazzetta-marshal-schulenburg.md)；书中位置 | S1 |

### 地点与使用

| 字段 | 值 | 证据 |
|---|---|---|
| 建筑性质 | 城堡建筑群／Castle complex | S2，PDF2–3 |
| 所在地 | [米兰](milan.md) | S2，PDF2 |
| 地址 | Piazza Castello | S2，PDF3 |
| 馆藏部门 | [斯福尔扎城堡素描室](../institutions/gabinetto-dei-disegni-castello-sforzesco.md) | S2，PDF3 |

## 关系与证据

### 身份与外部链接

[SIRBeC馆藏建筑与保管部门](https://www.lombardiabeniculturali.it/opere-arte/schede-complete/4y010-09081/)（S2，PDF2–3）。

### 关系记录

暂无正式关系。内容表中的知识元链接表示有来源的关联记录。

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
