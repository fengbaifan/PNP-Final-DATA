---
title: 罗马圣奥古斯丁圣殿（Basilica of Saint Augustine in Campo Marzio）
name_en: Basilica of Saint Augustine in Campo Marzio
type: place
created: '2026-09-13'
updated: 2026-09-14
evidence_status: source_backed
relations: []
sources:
- citation: Wikipedia (en), Caravaggio, revision 1372526492. https://en.wikipedia.org/wiki/Caravaggio. Accessed 2026-09-11.
  location: 与本端点有关的作品、人物或关系段；全文已保存于REV-052
  sentence_summary: 《洛雷托圣母》在其中卡瓦莱蒂礼拜堂安置。
- citation: Wikipedia (en), Sant'Agostino, Rome. https://en.wikipedia.org/wiki/Sant'Agostino,_Rome. Accessed 2026-09-13.
  location: 正文全文已保存于REV-068；本轮定向读取身份、类型及当前作品链字段
  sentence_summary: 核对Basilica of Saint Augustine in Campo Marzio的身份、类型及本轮关系角色。
- citation: Wikidata, Q1424095. https://www.wikidata.org/wiki/Q1424095. Accessed 2026-09-13.
  location: labels、descriptions、P31及适用标识／sitelink；不机械采纳全部声明
  sentence_summary: 用于Basilica of Saint Augustine in Campo Marzio的同粒度身份核对；QID不验证本卡全部事实。
process_ref: 03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072-places-sant-agostino-rome
---

## 内容

### 描述

**中文：** 罗马圣奥古斯丁圣殿是罗马教堂建筑。《洛雷托圣母》在其中卡瓦莱蒂礼拜堂安置。

**English:** Sant’Agostino in Rome is a church building containing the Cavalletti Chapel, where Madonna of Loreto is installed.

### 名称、性质与空间角色

| 字段 | 内容 | 依据 |
|---|---|---|
| 规范名 | 罗马圣奥古斯丁圣殿／Basilica of Saint Augustine in Campo Marzio | S1–S3（如有） |
| 地点性质 | 罗马教堂建筑 | S1–S3 |
| 所在地 | 罗马，意大利 | S2–S3 |
| 建筑／空间信息 | 天主教宗座圣殿；现存主体1483年完成 | S2–S3 |
| 本轮空间角色 | 《洛雷托圣母》在其中卡瓦莱蒂礼拜堂安置。 | S1 |

### 建筑、内部空间与作品

本卡只记录当前作品链所需的委托、原定安置、历史安置或现存地点；建筑与管理机构、城市和内部独立作品不合并。

## 关系与证据

### 关系记录
| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 安置地点（`installation_site_of`，反向投影） | [《洛雷托圣母》（Madonna di Loreto）](../works/caravaggio-madonna-loreto.md) | 来源将Madonna di Loreto与该实际或历史安置地点对应；时间：见作品字段；角色：安置地点；范围：现存；原断言与证据见发出端卡片“《洛雷托圣母》（Madonna di Loreto）”：[来源](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))；installed-at-places-sant-agostino-rome-md；本卡S2；有S4时并参对象／地点说明 |
| ← 预定地点（`intended_site_of`，反向投影） | [《洛雷托圣母》（Madonna di Loreto）](../works/caravaggio-madonna-loreto.md) | Madonna di Loreto原定用于该建筑或礼拜堂；时间：约1604–1606；角色：原定安置地点；范围：按卡瓦莱蒂礼拜堂对象记录；模特身份争议不转成确定关系。；原断言与证据见发出端卡片“《洛雷托圣母》（Madonna di Loreto）”：[来源](https://en.wikipedia.org/wiki/Madonna_di_Loreto_(Caravaggio))；intended-for-places-sant-agostino-rome-md；本卡S2；有S4时并参原定安置段 |

### 身份与外部链接

- [Wikipedia：Sant'Agostino, Rome](https://en.wikipedia.org/wiki/Sant'Agostino,_Rome)
- [Wikidata Q1424095](https://www.wikidata.org/wiki/Q1424095)
