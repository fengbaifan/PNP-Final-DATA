---
title: 圣阿萨夫（St Asaph）
name_en: St Asaph
type: place
evidence_status: source_backed
sources:
- citation: Francis Haskell, Patrons and Painters, revised and enlarged edition (Yale University Press, 1980; this
    printing 2006).
  location: 章前：图版目录；印刷页xv；OCR L125–125
  sentence_summary: 图版47目录将其记为书中位置。
  original_quotes:
  - source_span: lines 125–125
    text: '47 Marco Ricci: An Operatic Rehearsal (Sir Watkin Williams-Wynn, Bt, St Asaph)'
  evidence_ref:
    doc_id: patrons-and-painters
    source_file: 02-sources/02-Markdown/00_05_List_of_Plates.md
    source_span: lines 125–125; 章前：图版目录；印刷页xv
created: '2026-09-14'
updated: '2026-09-14'
process_ref: 03-processing/patrons-and-painters-front-matter/process/knowledge.md
---

## 内容

### 描述

**中文：** 图版47目录将其记为书中位置。

**English:** The entry for plate 47 records this entity in the role of location.

### 名称

| 字段 | 值 | 证据 |
|---|---|---|
| 原文名称 | St Asaph | S1 |
| 中文名性质 | 项目工作译名 | 项目命名 |

### 来源所载信息

| 字段 | 值 | 证据 |
|---|---|---|
| 图版47角色 | 书中位置 | S1 |
| 图版47相关对象 | [歌剧排练](../works/marco-ricci-an-operatic-rehearsal.md)；书中位置 | S1 |

## 关系与证据

### 关系记录

| 方向与关系 | 关联知识元 | 语境与证据 |
|---|---|---|
| ← 所在地（`location_of`，反向投影） | [歌剧排练（An Operatic Rehearsal）](../works/marco-ricci-an-operatic-rehearsal.md) | 时间：本书所述时点；角色：书中收藏地点；范围：原书目录位置；不外推现藏或画中地点；原断言与证据见发出端卡片“歌剧排练（An Operatic Rehearsal）”：[来源](../../../02-sources/02-Markdown/00_05_List_of_Plates.md)；patrons-and-painters；lines 125–125; 章前：图版目录；印刷页xv |

### 关系候选入口

[章前原文关系候选与端点映射](../../../03-processing/patrons-and-painters-front-matter/process/knowledge.md#候选登记映射)
