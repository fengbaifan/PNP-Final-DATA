...
**当前执行点（REV-072，2026-09-14）：九类418张有效实体卡已整理来源原文与过程说明。** 其中298张卡的437条原书来源补入534段带独立行号的原语言摘录；其余120张没有直接原书定位，不伪造章内出处。摘录来自既存OCR，保留原字和断行（去除行末空格），尚不能称扫描原页逐字校勘。对齐、补足及重复来源导读移入[既有过程记录](../../03-processing/patrons-and-painters-chp-1/process/knowledge.md#rev-072来源原文与实体说明整理2026-09-14)，各卡process_ref可定位原说明；卡内保留事实字段、必要限定、身份链接与正式关系。60张作品卡的登记式描述改为事实式双语描述。

**本次验证：** 534段摘录逐段与所指OCR行核对；来源原有字段、来源顺序及frontmatter关系集合保持不变，内容结构检查无机械缺陷，关系断端点／非法类型／缺反向／弱证据均为0。当前文件头实际有407条关系记录，809行关系视图；既往406条加1条的分类口径与当前407条均标explicit不一致，需在后续关系语义审查中核定，不能把此次格式整理当作新增关系或独立语义验收。论文标注验证尚未开展；知识发现、页面刷新与第六章继续暂停。
...
| 初步对齐 | Wikipedia—Wikidata身份配对由172增至175；原126个无同粒度配对对象保持原处置，不以缺QID判定失败 |
