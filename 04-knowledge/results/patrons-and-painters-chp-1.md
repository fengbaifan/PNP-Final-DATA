...
**当前执行点（REV-065，2026-09-12）：REV-064检查出的具体问题已修复；第一章第一部分仍为部分完成。** 第一章当前有效301个知识元、226条正式有向关系。81条宽泛关系已逐条改为具体语义，关系限定和证据回链已进入卡片与索引；补齐3个家族／婚姻必要端点，卡拉瓦乔人物卡的Wikipedia具名对象清单校正为57项。现有成果可继续使用，但已采纳内容中的其余具名作品尚未全部建立KU和关系，因此不把本轮修复写成第一部分全部完成。
