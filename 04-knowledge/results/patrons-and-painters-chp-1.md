...
**当前进度（2026-09-26）：第一章样例的知识元补足与关系审查继续进行，尚未整体定稿。** S2 结构化迁移的 27 个已审阅段全部完成（另 3 个格式标题排除），包含 665 条实体提及和 172 条原书断言；严格阶段审计现无结构或迁移错误。另定向对照四个未纳入既有盲审样本的段落（75条提及、23条断言），将 “Northern Europe” 从宽泛的 Europe 候选中拆出，新增开放地域候选 `cand-3594`；该同上下文抽查不构成独立验收或准确率估计。脚注复核将 OCR L321–322 的 Savini、Minnitti 提及归回实际承载正文的 `l28-38`，并把 L323–330 的其余来函记录保留在 `l168-239`。十二个正文段抽查较原账本补33条提及和19个开放候选：包括市场中介角色、St Luke、既有术语映射，以及 `sec_ii:l19-26` 中 subsidy 与赞助人优先权构成的补助机制、`sec_ii:l40-46` 中 movable gallery picture 和 connoisseurship、`sec_ii:l48-60` 中 Caravaggesque genre scenes、`sec_ii:l62-71` 中 Holy Years、Baroque period 和 amateur、`sec_ii:l73-77` 中 canvas，以及 `sec_ii:l79-84` 中第二处 deposit 和身份未定的 Duke of Mantua 独立候选；`sec_ii:l86-94` 中 dilettantes、三个地域性称谓、art dealing、art exhibitions、landscapes 与 large historical scenes；`sec_ii:l96-104` 中艺术家社会地位、Renaissance、Baroque、Bolognese 和 High Renaissance；未将泛称转写成具体人物 KU，也未仅凭头衔合并同名候选。逐段迁移完成不等于召回率、准确率或语义质量已经验证；隔离上下文的独立代理已完成分层9段盲审与交叉核对（363条现有提及、69条断言），按证据修正4处高置信错误；未据此估算全章precision/recall，也不作为正式人工验收；另在 `sec_ii:l118-127` 修正覆盖台账漏列的 OCR L717–722 并补两条断言；对 `l148-153`、`l155-159`、`l161-166` 的语义复核新增16条提及和10个开放候选。具体范围、原文锚点和限制见[摄入与处理过程](../../03-processing/patrons-and-painters-chp-1/process/stages.md)。全库现有1,019个有效KU、1,227条关系索引（1,225条正式、2条待证）。665条提及后的严格表审计通过；本轮全量同步闭环已通过（9项检查、规则契约4/4、293项测试及2个子测试）；结构闭环不替代语义准确率和召回率评估。关系机械检查的断端点、非法关系类型、缺反向映射和弱证据均为0。全库有207个单一来源KU、135个零关系KU，这些只是待结合语境审查的线索，不直接等于缺陷。本轮据Vincenzo Ruffo 1916年刊载的Guercino书信及注释，为圭尔奇诺西西里祭坛画补入委托者D.a Maria Ruffo端点和`commissioned_by`关系；至迟1664-04-08已进入委托协商，但修女名与Suor Illuminata是否相同、修院法人规范名仍未定。另补入Ludovisi亲缘链缺失的Orazio、Niccolò、Lavinia三个端点，完成其中两人的Wikipedia—Wikidata双向身份核对，并登记9条有据关系；Orazio卒年异文保留，Lavinia无可确认的个人Wiki身份。另补入Maggiotto肖像组、Maffeo Pinelli及Pisani di Santo Stefano支系三个KU和三条关系；Tommaso、Galeazzo Fontana两个亲缘端点及三条关系；Bernini家族人物KU及9条关系；《还愿祭坛画》、圣乌巴尔多圣殿和古比奥历史市政共同体三个KU及4条关系。1663-06-02 De Rosis书信及附笺描述画作另有两个KU；Ruffo 1916刊本第169页OCR现已核读，更新为书信作者、收信人、所涉画作、创作者和历史购藏共5条关系。刊本署记Roma、Bonhams称Messina的冲突保留；其所述画作与Bonhams lot 26现存画布是否同一仍待Volpi第553页等证据核对。作品无适用QID。另已读取孔代博物馆PE 305官方记录，补充普桑同题作候选的语境；因本章缺少版本锚点，候选仍不升级为确定身份。机械闭包不替代语义定稿。S5 全类型字段表为10,149行、1,501行为unverified；逐条复建发现两条旧卡来源标记S3–S4此前未写入表，现仅补回 `source_ref`，ID集合不变、重跑零差异。该来源范围超出两张卡各自登记的source清单，保持未解析并由严格审计报告，不猜补登记；另1,499条unverified记录没有source_ref；字段盘点显示其中83条另有source IDs／引文／URL，464条的evidence标记为“项目命名”，469条带Wikidata相关 evidence 标记但无结构化来源元数据，377条为其他evidence文本且无结构化来源元数据，106条的evidence与结构化来源字段均为空。进一步按字段标记区分，469条中465条为Wikidata属性式记录；403条可经唯一same对齐映射到52个QID，但每个QID在sources.csv中有两条记录；44组修订号相同、8组修订号不同，因此没有自动回填来源ID。其余62条缺少唯一same身份对齐，另4条虽带Wikidata字样但不是属性标记。身份对齐不构成属性值核验；草案包新增只读 `wikidata-source-candidates.csv`，列出465条待复核记录、候选 QID/来源记录及版本，明确不写入事实来源字段；包与具体限制见[数据包验证报告](../../release/v0.2-draft/validation-report.md)。

**卡片批量渲染（2026-09-26）：** 经用户明确授权，`build_cards.py --render` 在全库预检通过后按 `enrichment.jsonl` 重建结构化表行、按 `relations.csv` 重建关系行；保留卡片已有小节、表头/分隔行、关系说明与散文。1,019 张卡、2,608 张表、10,149 行完成写回并逐文件回读核对。随后 `--check` 幂等通过，待写文件为 0。此项是结构化数据到卡片视图的机械一致性，不代表 1,019 个 KU 的事实已逐条独立语义验收；严格表审计仍提示 1,501 条 enrichment 未核验、2 条来源定位未解析，草案发布元数据与第三方权利仍待处理。

**数据包复用准备（2026-09-26）：** 批量渲染后刷新 `release/v0.2-draft/`，验证报告记录当前零差异；新增可移植内链登记后，导出包含19个文件且外键与结构检查通过。`portable-references.csv` 登记3,781处 enrichment 内链，其中3,751处解析为稳定 KU ID、30处明确标记为未打包的过程记录；验证器检查引用覆盖与目标身份。`enrichment-citation-links.csv` 对12,254条引文逐条使用规范化精确文本或严格作品身份规则映射来源登记，9,328条规范化精确匹配、另2,873条以作者／书名／年份唯一匹配，53条仍未匹配；书目匹配不等于事实核验。标准库查询示例提供精简摘要、`--full` 完整行及内链查询，并修复 Windows 控制台 UTF-8；Guercino 查询可读出35条字段及1条出向、4条入向关系。另新增逐组件版权／引用清理清单，但不替代权利裁定。对 S0/S1/S5、严格表审计、render 预检与导出预览各重复运行五次，记录本机热缓存中位数与范围；这些是解析、核对、验证和序列化时间，不是语义抽取吞吐或前后加速率。作者字段按用户要求留空；此包仍是非公开草案，许可、持久标识和最终引用仍未确定；不据此声称数据集论文复用要求已全部满足。

**关系来源登记补链（2026-09-26）：** `relations.csv` 的1,227条关系中，原有740条未填 `source_id`；按精确且唯一的来源URL匹配补入622条来源ID。105条因来源表存在多条同URL记录而保持歧义，13条没有精确匹配；均未自动择源。写后逐字段核对确认仅622个来源ID单元格变化；关系卡片写回后 `build_cards.py --check` 为零待写差异。此映射提升关系数据与 `sources.csv` 的可连接性，不替代关系事实的独立语义核验。
