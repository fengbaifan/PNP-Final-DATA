...
S0 输入：`02-sources/02-Markdown/` 中的分节文件（不读整章文件）。
S1 输入：`02-sources/03-Index/03-2-Index-CSV/`、`00_05_List_of_Plates.md`、`21_CHP-21Bibliography.md`。
S2 输入：`segments.jsonl` 和 `entity-candidates.csv`；完整阅读正文、注释与必要图像，同步记录原文关系候选、提及、指代、证据跨度及语气／发言者限定，不提前写成正式边。
S4 输入：`alignment.csv` 中 `decision=new` 的行（新建 KU），以及 `decision=same` 的行（复用已有 KU，只更新映射）；检索现有对象判断同一性、类型和边界，端点不明时保留待决，不由关系猜造实体。
