...
REV-041、042 的“分字段”指正文中按类型独立表达属性、多值列表和时间条目，具体格式只在 body-template.md 维护；不把九类内容属性搬进本元数据契约，不增加第二份属性 YAML/JSON。来源登记及现有关系接口不变；详细采集／裁决过程在 03，卡内保留可追溯引用和当前争议。
...
sources:
  - citation: 来源书目信息
    ...
    original_quotes:
      - source_span: "lines 起始行–结束行"
        text: |-
          对应原书原文，保留原语言和实际断行；不同位置分别列条。
    evidence_ref:
      doc_id: 实际来源标识
      source_file: 实际文件路径
...
按 REV-072，原书来源在 sources.original_quotes 分条保存原语言摘录及各自精确 source_span；句意摘要与原文并存，不将摘要反译为引文。不连续位置不拼成连续原句；需要补全句界时回读上下文并记录扩展后的真实定位，跨页须区分正文、脚注和页眉。OCR 摘录不得冒称扫描页逐字校勘，校勘与断行处置记入03过程。外部补足对象没有原书直接出处时不伪造引文；该字段只用于实际具有原书出处的来源，不用空引文占位。process_ref 可链接该卡的必要过程记录。
元数据登记“引用了什么”，内容属性/记录须注明“哪一出处支持这一信息”；整卡 evidence_status 不能代替事实级的支持范围。第三部分“关系与证据”（REV-020）只展示简明身份入口、关系、必要事实限定及证据引用，不重复来源摘要、阅读/对齐/补足过程或完成声明。现有 relations 在 frontmatter 中保留唯一机器记录，逻辑上属于第三部分，不是一般来源元数据。正文按 body-template.md 展示身份和关系链接，不搬动机器记录或增设同义字段；可读展示不等于第二套关系数据。
