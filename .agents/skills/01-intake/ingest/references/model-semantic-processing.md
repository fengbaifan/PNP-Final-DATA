...
`status` 只能是 `novel`、`existing_target`、`potential_duplicate`、`potential_conflict` 或 `undetermined`。脚本可以召回精确名称、slug、claim 主体和 relation 端点，但不得自动裁决语义重复或矛盾。`potential_conflict` 交给 `reconcile`；`undetermined` 只能保留为待证据或延期。
