...
| unit create / merge | `ingest` 或 `reconcile` exact change-set | 模板和字段可机械写入，类型与合并裁决不可自动化 |
...
| theme / topic | `synthesize` boundary test -> `evolve-hierarchy` | 不自动宣布成熟或写入 hierarchy |
| cluster | `synthesize` boundary test | 只批准为发现信号；不得物化为 structure node，正式结果另建类型化候选 |
| hierarchy_change | `evolve-hierarchy` 专项变更 | 必须记录 impact 与 unresolved items |
