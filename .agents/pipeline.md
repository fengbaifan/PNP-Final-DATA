...
每阶段只产一个命名产物（交接物），下游只读该产物、不重跑上游。目标：S0–S6 产物统一存放在 `04-knowledge/tables/`，是结构化事实的唯一权威，`accepted.yml` 和卡片 `units/*.md` 的结构化部分是其生成视图。过渡期现状：`build_tables.py` 仍从卡片 frontmatter 与 `accepted.yml` 导出这些表，卡片仍是脚本实际读取的源，完整反转（tables 为源）待做。`release/vX/` 是 S7 的冻结快照。视图和快照都不是事实源。
